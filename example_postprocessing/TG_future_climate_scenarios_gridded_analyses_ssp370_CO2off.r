...
late_GS_correlation = "n"
Cfol_Croot_ratio_source=" "
# Update the user
...
output_suffix = "_CO2off" # must include "_" at the beginning
# Location to place the outputs of this script
outdir = "~/gcel/for_tim/future_climate_outputs/check/"
...
#      wind_spd_bias = wind_spd_bias + wind_spd_ms
      ...
      new_drivers$met[start:finish,5]  = rep(c(drivers$met[(dim(drivers$met)[1]-11):dim(drivers$met)[1],5]),nos_years_anomaly) # loops the last year of the contemporary CO2 TG # co2_bias = CO2 on
