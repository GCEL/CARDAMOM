...
infofile = paste(load_paths$cardamom_outputs,"/DALEC.A1.C1.D2.F2.H2.P1.#_MHMCMC/Fin_strat_mineral_01degree_SSedcyes_LL_EFDA_harvestx2_FINAL/infofile.RData",sep="")
...
#outdir_drivers <- "/exports/csce/datastore/geos/groups/gcel/for_tim/future_climate_drivers/"
...
run_each_site_local<-function(site,ssp,PROJECT,outdir,original_end,cardamom_lat,cardamom_long,
  ...
  #driver_outfile =  paste(outdir_drivers,PROJECT$sites[site],"_",ssp_scenarios[ssp],"_",ESM,output_suffix,"_drivers.RData",sep="")           
...
#      wind_spd_bias = wind_spd_bias + wind_spd_ms
      ...
      new_drivers$met[start:finish,5]  = rep(c(drivers$met[(dim(drivers$met)[1]-11):dim(drivers$met)[1],5]),nos_years_anomaly) # loops the last year of the contemporary CO2 TG
...
#      new_drivers$met[start:finish,15] = wind_spd_bias
	  ...
	  # save new drivers TG
	  #save(new_drivers,file=driver_outfile)
