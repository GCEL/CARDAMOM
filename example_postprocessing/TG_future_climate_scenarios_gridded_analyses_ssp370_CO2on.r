...
late_GS_correlation = "y"
Cfol_Croot_ratio_source="Helmisaari"
# Update the user
...
infofile = paste(load_paths$cardamom_outputs,"/DALEC.A1.C1.D2.F2.H2.P1.#_MHMCMC/Fin_strat_mineral_LL_EFDA_harvestx2_maxroot_kallio_CfolCroot_Cfol/infofile.RData",sep="")
# Load project for climate
load(infofile)

# Suffix for the output files
output_suffix = "_CO2on" # must include "_" at the beginning
# Location to place the outputs of this script
outdir = "~/gcel/for_tim/future_climate_outputs/"
#outdir_drivers <- "/exports/csce/datastore/geos/groups/gcel/for_tim/future_climate_drivers/" #TG
...
ssp_scenarios = "ssp370" #c("ssp119","ssp126","ssp434","ssp245","ssp370","ssp585")
...
run_each_site_local<-function(site,ssp,PROJECT,outdir,original_end,cardamom_lat,cardamom_long,
  ...
  # Define the output variables name        TG           
  outfile_stock_fluxes = paste(outdir,PROJECT$sites[site],"_",ssp_scenarios[ssp],"_",ESM,output_suffix,".RData",sep="")
  #driver_outfile =  paste(outdir_drivers,PROJECT$sites[site],"_",ssp_scenarios[ssp],"_",ESM,output_suffix,"_drivers.RData",sep="")  #TG          
...
#      harvest = grid_harvest[i1,j1,]
	  ...
	  ##### comment out to keep "baseline" harvest TG, uncomment for intense harvest (1.2*base)
      #new_drivers$met[(dim(drivers$met)[1]+1):dim(new_drivers$met)[1],8] = new_drivers$met[(dim(drivers$met)[1]+1):dim(new_drivers$met)[1],8]*1.2
...
#      wind_spd_bias = wind_spd_bias + wind_spd_ms
      ...
      new_drivers$met[start:finish,5]  = co2_bias#rep(c(drivers$met[(dim(drivers$met)[1]-11):dim(drivers$met)[1],5]),nos_years_anomaly) # loops the last year of the contemporary CO2 TG # co2_bias = CO2 on
...
#      new_drivers$met[start:finish,15] = wind_spd_bias
	  ...
	  # save new drivers TG
	  #save(new_drivers,file=driver_outfile) # TG
	  ...
	  ####################################
      if (dummy == 0) { return(outfile_stock_fluxes) } #TG - uncomment after met extraction complete
      ####################################
	...
	} else if(file.exists(outfile_stock_fluxes) == TRUE){
		return(outfile_stock_fluxes)					  
...
#         grid_harvest = grid_harvest / (365.25 / mean(PROJECT$model$timestep_days))
         ...
         co2_ppm = co2_ppm - co2_ppm[end_anomaly_esm]
