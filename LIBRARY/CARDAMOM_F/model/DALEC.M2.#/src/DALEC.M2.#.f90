...
!          22. labile loss (per time step)    
!          23. foliage loss (per time step)
...
!         *9.burnt area fraction                           ! 34. Minimum grazed biomass to allow grazing
...
!
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,LAI,NEE,FLUXES,POOLS &
                         ,nopars,nomet,nopools,nofluxes,GPP,REMOVED_C) ! CHANGE ALL LAI to lai
    ! CARBON_MODEL of DALEC_GRASS has two more parameters 'REMOVED_C' and 'version_code'.
    ! For DALEC.M2.# SZ removed version code, because no need for site-level LSU run for integrated DALEC_GRASS
    ! The parameter is 'LAI' instead of 'lai'
    ...
    !f2py intent(out) ::LAI,NEE,FLUXES,POOLS,GPP,REMOVED_C    
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do n = start, finish  
      ...
      ! if AGB > limit & LAI > 3 & LAI reduction = -1 & no cut in past month 
      if ( ((POOLS(n+1,2)+POOLS(n+1,1)) .GE. (pars(28)*0.0475))   & 
            .AND. ( met(6,n) .GE. 91 ) .AND. ( met(6,n) .LE. 304 ) & 
            ! .AND. ( LAI(n) .GE. 3 ) & 
            .AND. ( met(8,n) .EQ. -1 ) &
            .AND. ( REMOVED_C(2,n-1) .EQ. 0 ) .AND. ( REMOVED_C(2,n-2) .EQ. 0 ) &
            .AND. ( REMOVED_C(2,n-3) .EQ. 0 ) .AND. ( REMOVED_C(2,n-4) .EQ. 0 ) ) then
          ...
          if ( ((foliar_loss-foliar_residue)+(labile_loss-labile_residue)) .GE. (1500*0.0475) ) then
            ...
            ! labile loss due to grass cutting - SZ 01/11/2023
            FLUXES(n,22) = labile_loss
            
            ! foliar loss due to grass cutting - SZ 01/11/2023
            FLUXES(n,23) = foliar_loss
      ...
      ! if LAI reduction > 0 & AGB > pregraze limit & no cut this and last 2 weeks
      if ( (met(8,n) > 0.0) .AND. ( (POOLS(n+1,2)+POOLS(n+1,1)) .GE. (pars(27)*0.0475) )  & 
          .AND. (REMOVED_C(2,n) .EQ. 0.0) .AND. (REMOVED_C(2,n-1) .EQ. 0.0) .AND. (REMOVED_C(2,n-2) .EQ. 0.0) & 
          .AND. ( met(8,n+1) .NE. -1 ) .AND. ( met(8,n+2) .NE. -1 ) ) then
