...
!          14.respiration het som                          ! 15. LCA (g.C.leaf_m-2) -OR-slope value for LCA = -slope*LAI+intercept  
...
!           - field mode = LSU.ha-1 or 100 for cutting     ! 33. Post-cut labile loss (fraction)
...
!         *14.Mean T
! ----------------------------------------------------------------------------------------------------------------
! NOTES : '*' means the pool/flux is not used/applicable for grasslands 
    ...
    do n = start, finish  
      ...
      ! -------------------------------- CUTTING--------------------------------------------------- ! 
        ...
        if ( ((POOLS(n+1, 2)+POOLS(n+1, 1)) .GE. (pars(28)*0.0475))   &  ! AGB >= pre-cutting limit 
             ...
             ! .AND. ( met(8, n) .GE. 2 ) &                               ! removed LAI >= 2 m2.m-2
             .AND. ( (met(8, n) * LMA*0.021) .GE. 1 ) &                 ! removed leaf biomass >= 1 tDMha-1
