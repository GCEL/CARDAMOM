...
! of DALEC.A1.C1.D2.F2.H3.P1 as a function of observations and ecological dynamical constraints.
!
...
!        enddo
!          ! Combine with existing likelihood estimate
!        scale_likelihood = scale_likelihood-(tot_exp/dble(DATAin%nmLWP))
!    endif
...
!print*,"log_scale_likelihood: Fire done"
  ...
  !        enddo
  !          ! Combine with existing likelihood estimate
  !        log_scale_likelihood = log_scale_likelihood-(tot_exp/(1d0+log(dble(DATAin%nmLWP))))
  !    endif
...
!------------------------------------------------------------------
!
! subroutine log_model_likelihood_dtm(PARS,ML_obs_out,ML_prior_out)
!     use MCMCOPT, only:  PI
!     use CARBON_MODEL_MOD, only: carbon_model
!     use cardamom_structures, only: DATAin
...
! !    ! Debugging print statement
! !    print*,"log_scale_likelihood_dtm: "

!     ! initial value
!     log_scale_likelihood_dtm = 0d0 ; infini = 0d0 ; mid_state = 0d0 ; sub_time = 0d0
...
!         end do ! loop years
!         ! Update the likelihood score with the anomaly estimates
!         log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nnbe)))/dble(DATAin%nnbe))
!     endif ! nnbe > 0
! !print*,"log_scale_likelihood_dtm: NBE done"
...
! !          end do
!         ! Update the likelihood score with the anomaly estimates
!         log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%ngpp)))/dble(DATAin%ngpp))
!     endif
...
! !         tot_exp = tot_exp+((tmp_var-DATAin%Evap(dn))/DATAin%Evap_unc(dn))**2
! !       end do
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nEvap)))/dble(DATAin%nEvap))
!     endif
...
!         end do ! loop years
!         ! Update the likelihood score with the anomaly estimates
!         log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nFire)))/dble(DATAin%nFire))
!     endif
! !print*,"log_scale_likelihood_dtm: Fire done"
!     ! LAI log-likelihood
!     ! Assume physical property is best represented as the mean of value at beginning and end of times step
!     if (DATAin%nlai > 0) then
!        ! Create vector of (LAI_t0 + LAI_t1) * 0.5, note / pars(17) to convert foliage C to LAI
...
!          endif
!        end do
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nlai)))/dble(DATAin%nlai))
!     endif
...
!        !  tot_exp = tot_exp+((tmp_var-DATAin%NEE(dn))/DATAin%NEE_unc(dn))**2
!        !end do
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nnee)))/dble(DATAin%nnee))
!     endif
...
!        end do
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nreco)))/dble(DATAin%nreco))
!     endif
...
!     ! Foliage to leaf litter log-likelihood
!     if (DATAin%nfoliage_to_litter > 0) then
!         tot_exp = 0d0
!         do n = 1, DATAin%nfoliage_to_litter
!           dn = DATAin%foliage_to_litterpts(n)
...
!                      / DATAin%Cfol_stock_unc(DATAin%Cfol_stockpts(1:DATAin%nCfol_stock)))**2)
!        ! Sum with current likelihood score
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nCfol_stock)))/dble(DATAin%nCfol_stock))
!     endif
...
!          tot_exp = tot_exp+((tmp_var-DATAin%Cfolmax_stock(dn)) / DATAin%Cfolmax_stock_unc(dn))**2
!        end do
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nCfolmax_stock)))/dble(DATAin%nCfolmax_stock))
!     endif
...
!                      / DATAin%Cwood_stock_unc(DATAin%Cwood_stockpts(1:DATAin%nCwood_stock)))**2)
!        ! Combine with existing likelihood estimate
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nCwood_stock)))/dble(DATAin%nCwood_stock))
!     endif
...
!                      / DATAin%Croots_stock_unc(DATAin%Croots_stockpts(1:DATAin%nCroots_stock)))**2)
!        ! Combine with existing likelihood estimate
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nCroots_stock)))/dble(DATAin%nCroots_stock))
!     endif
...
!                      / DATAin%Clit_stock_unc(DATAin%Clit_stockpts(1:DATAin%nClit_stock)))**2)
!        ! Combine with existing likelihood estimate
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nClit_stock)))/dble(DATAin%nClit_stock))
!     endif
...
!                      / DATAin%Csom_stock_unc(DATAin%Csom_stockpts(1:DATAin%nCsom_stock)))**2)
!        ! Combine with existing likelihood estimate
!        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nCsom_stock)))/dble(DATAin%nCsom_stock))
!     endif
...
! !        enddo
! !          ! Combine with existing likelihood estimate
! !        log_scale_likelihood_dtm = log_scale_likelihood_dtm-(tot_exp*(1d0+LOG(dble(DATAin%nmLWP)))/dble(DATAin%nmLWP))
! !     endif
!     ! the likelihood scores for each observation are subject to multiplication
!     ! by 0.5 in the algebraic formulation. To avoid repeated calculation across
!     ! multiple datastreams we apply this multiplication to the bulk likelihood
!     ! hear
!     log_scale_likelihood_dtm = log_scale_likelihood_dtm * 0.5d0

!     ! check that log-likelihood is an actual number
!     if (log_scale_likelihood_dtm /= log_scale_likelihood_dtm) then
!         log_scale_likelihood_dtm = log(infini)
!     end if
...
!   end function log_scale_likelihood_dtm
  !
  !------------------------------------------------------------------
  !
