...
! This file contains the source code of DALEC.A1.C1.D2.F2.H3.P1
!
...
! D. T. Milodowski (University of Edinburgh, d.t.milodowski@ed.ac.uk)
      ...
      !! For a single chain's model_working_varibles type object mV, allocate arrays
       ...
       ! allocate variables dimension which are fixed per site only the once
         allocate(mV%deltat_1(nodays),mV%wSWP_time(nodays),mV%rSWP_time(nodays),mV%gs_demand_supply_ratio(nodays), &
                 ...
                 mV%Reff_time(nodays),mV%LWP_time(nodays))
...
!    print*,"carbon_model: "
    ...
    ! minimum LWP
    mV%minlwp = pars(33) ! minimum LWP (MPa)
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    mV%snow_storage_time(n) = mV%snow_storage
       ...
       mV%gb_total_canopy(n) = mV%aerodynamic_conductance * mV%convert_ms1_mmol_1 * &
       ...
       mV%canopy_par_MJday_time(n) = mV%canopy_par_MJday
       mV%soil_par_MJday_time(n) = mV%soil_par_MJday
       ...
       mV%wSWP_time(n) = mV%wSWP ; mV%rSWP_time(n) = mV%rSWP ; mV%root_depth_time(n) = mV%root_reach; mV%Reff_time(n) = mV%Reff
       ...
       mV%gs_demand_supply_ratio(n) = (mV%stomatal_conductance  - mV%minimum_conductance) &
       ...
       mV%gs_total_canopy(n) = mV%stomatal_conductance
       ...
       if (mV%stomatal_conductance > vsmall) then
           ...
           ! assume zero fluxes
           FLUXES(n,1) = 0d0 ; transpiration = 0d0 ; mV%cica_time(n) = 0d0
       endif
       ...
       mV%mLWP =  mV%wSWP - (((transpiration*mV%dayl_seconds_1)/mmol_to_kg_water) * mV%Reff)
       mV%LWP_time(n) = mV%mLWP
...
!    endif
  ...
  subroutine calculate_Rtot (mV)
    ...
    ! reset water flux
    mV%total_water_flux = 0d0 ; mV%water_flux_mmolH2Om2s = 0d0 ; mV%mLWP = 0d0; mV%wSWP = 0d0 ; mV%rSWP = 0d0
    slpa = 0d0 ; root_length = 0d0 ; root_mass = 0d0 ; mV%Rcond_layer = 0d0 ; mV%conductance_mmolH2OMPam2s = 0d0
...
!(((rmax / rd) - 1)**(1/c)) * d50 = d ! Depth at which rd = 99 %
    ...
    do i = 1, nos_root_layers
       ...
       if (root_mass(i) > 0d0) then
           ...
           call plant_soil_flow(i,root_length(i),root_mass(i) &
                               ...
                               ,transpiration_resistance,mV%Rcond_layer(i), mV)
       else
    ...
    if (mV%total_water_flux <= vsmall) then
        ...
        mV%rSWP = sum(mV%SWP(1:rooted_layer) * (mV%Rcond_layer(1:rooted_layer) / sum(mV%Rcond_layer(1:rooted_layer))))
    endif
...
!    endif
  ...
  subroutine plant_soil_flow(root_layer,root_length,root_mass &
    ...
    !
    ! Calculate soil layer specific water flow from the soil to canopy (mmolH2O.m-2.s-1)
    ...
    if (demand > 0d0) then
      mV%conductance_mmolH2OMPam2s(root_layer) = 1d0/Rtot_layer
    else
      mV%conductance_mmolH2OMPam2s(root_layer) = 0d0
    endif
