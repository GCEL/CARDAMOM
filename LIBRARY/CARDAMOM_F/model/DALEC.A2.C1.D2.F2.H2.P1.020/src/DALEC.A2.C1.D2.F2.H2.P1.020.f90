...
! This file contains the source code of DALEC.A2.C1.D2.F2.H2.P1.020
!
...
module CARBON_MODEL_MOD
  ...
  subroutine initialize_mv(mV, nodays, nomet, nopars, deltat, soil_frac_sand, soil_frac_clay, met, lat)
    ...
    allocate(mV%deltat_1(nodays),  &
             ...
             mV%rainfall_time(nodays), mV%airt_zero_fraction_time(nodays))
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       if (mV%stomatal_conductance > vsmall) then
           ...
           FLUXES(n,1) = (acm_gpp_stage_2(mV%stomatal_conductance, mV) + mv%dark_respiration) * umol_to_gC * mV%dayl_seconds
       ...
       FLUXES(n,9)  = ff_wf*exp(-(sin((mV%doy-pars(15)+osf)/sf)*sf_over_wf)**2d0)
       FLUXES(n,16) = fl_wl*exp(-(sin((mV%doy-pars(12)+osl)/sf)*sf_over_wl)**2d0)
...
!    print*,"carbon_model: done"
  ...
  subroutine acm_gpp_stage_1(mV)
    ...
    !mV%dark_respiration = 0.002d0 * Vcmax_ref * airt_adj * leaf_canopy_light_scaling
    ...
    !mV%dark_respiration = (lai * Rm_heskel_polynomial(Rm_leaf_const,leafT)) 
    !mV%dark_respiration = (leaf_canopy_light_scaling * Rm_heskel_polynomial(Rm_leaf_const,leafT)) 
    ...
    ! R2 of fit 0.22
    !RLVratio = 0.036d0 + (-0.001d0 * mV%meant)
    !mV%dark_respiration = mV%Vcmax_ref * airt_adj * mV%leaf_canopy_light_scaling * RLVratio
    ...
    !Vcmax = (((mV%avN * mV%lai) / mV%leaf_canopy_light_scaling) * mV%NUE) * airt_adj * mV%leaf_canopy_light_scaling
