...
! This file contains the source code of DALEC.A1.C2.D2.F2.H2.P2.R3
!
...
module CARBON_MODEL_MOD
  ...
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,FLUXES,POOLS,DIAGS &
    ...
    ! 17 = total ecosystem fire emission - sum(18:23,32) (gC/m2/day)
    ...
    ! 33 = fire mortality transfer wood litter->som (gC/m2/day)
    ...
    ! 38 = harvest extraction from litter (gC/m2/day)
    ! 39 = harvest extraction from wood litter (gC/m2/day)
    ! 40 = harvest extraction from som (gC/m2/day)
    ! 41 = harvest litter residue from labile (gC/m2/day)
    ! 42 = harvest litter residue from foliage (gC/m2/day)
    ! 43 = harvest litter residue from fine roots (gC/m2/day)
    ! 44 = harvest litter residue from wood (gC/m2/day)
    ! 45 = total harvest extracted C - sum(34:44) (gC/m2/day)
    ...
    ! 55 = leaf maintenance respiration (Heskel polynomial) plus growth respiration from labile->foliage transfer (gC/m2/day)
    ...
    ! p(34) = initial wood litter C pool (gC/m2)
    ! p(35) = wood litter turnover rate, temperature adjusted (fraction/day)
    ! p(36) = half saturation coefficient for self-thinning suppression on wood turnover (gC/m2)
    ! p(37) = baseline leaf maintenance respiration coefficient (Heskel polynomial)
    ! p(38) = labile pool lifespan (yr)
    ...
    fl = (log(pars(38))-log(pars(38)-1d0)) * 0.5d0
    ! offset for labile and leaf turnovers
    osf = ospolynomial(pars(5),wf)
    osl = ospolynomial(pars(38),wl)
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       FLUXES(n,55) = seconds_per_day * umol_to_gC * mV%lai * Rm_heskel_polynomial(mV%Rm_leaf_const,mV%leafT,mV%cold_shutdown)
       ...
       available_labile = POOLS(n,1) + ((FLUXES(n,5)-FLUXES(n,55)) * mV%days_per_step)
       if (available_labile < 0d0) then
           ! Convert into per day rate and absolute value for easier mass balance comparisons
           available_labile = abs(available_labile / mV%days_per_step)
           ! Assess options for making the mass-balance
           if (FLUXES(n,4) > available_labile) then
               ! Take C needed from the direct to foliage allocation
               ! and give to the labile production
               FLUXES(n,5) = FLUXES(n,5) + available_labile
               FLUXES(n,4) = FLUXES(n,4) - available_labile 
           else if (FLUXES(n,4)+FLUXES(n,7) > available_labile) then
               ! Take C needed from the direct to foliage allocation and wood growth,
               ! and give to the labile production
               FLUXES(n,5) = FLUXES(n,5) + available_labile ! increase allocation to labile
               available_labile = available_labile - FLUXES(n,4) ! redirect all direct foliar growth to labile
               FLUXES(n,7) = FLUXES(n,7) - available_labile ! redirect the remaining balance from wood production
               FLUXES(n,4) = 0d0 ! Zero direct foliar allocation
           else if (FLUXES(n,4)+FLUXES(n,6)+FLUXES(n,7) > available_labile) then
               ! Take C needed from the direct to foliage allocation, wood growth and fine root growth,
               ! and give to the labile production
               FLUXES(n,5) = FLUXES(n,5) + available_labile ! increase allocation to labile
               available_labile = available_labile - FLUXES(n,4) ! redirect all direct foliar growth to labile
               available_labile = available_labile - FLUXES(n,7) ! redirect all wood growth to labile
               FLUXES(n,6) = FLUXES(n,6) - available_labile ! redirect the remaining balance from root production
               FLUXES(n,4) = 0d0 ! Zero direct foliar allocation            
               FLUXES(n,7) = 0d0 ! Zero wood allocation            
           else 
               ! Total C allocation, less that to wood+root maintenance respiration, cannot cover demand for leaf maintenance.
               ! Therefore, we must take all the C we can, plus lose some canopy.
               FLUXES(n,5) = FLUXES(n,5) + available_labile ! increase allocation to labile
               available_labile = available_labile - FLUXES(n,4) ! redirect all direct foliar growth to labile
               available_labile = available_labile - FLUXES(n,6) ! redirect all root growth to labile
               available_labile = available_labile - FLUXES(n,7) ! redirect all wood growth to labile
               FLUXES(n,6) = FLUXES(n,6) - available_labile ! redirect the remaining balance from wood production
               FLUXES(n,4) = 0d0 ! Zero direct foliar allocation            
               FLUXES(n,6) = 0d0 ! Zero wood allocation                        
               FLUXES(n,7) = 0d0 ! Zero root allocation                        
               ! Now induce mortality needed to balance the remaining balance.
               ! First, the per gC cost of leaves
               tmp = seconds_per_day * umol_to_gC * (Rm_heskel_polynomial(mV%Rm_leaf_const,mV%leafT,mV%cold_shutdown) / pars(17))
               ! Second, how many gC need to be lost to balance the demand
               tmp = available_labile / tmp
               ! Third, convert to a fraction of the labile pool and, make per day
               tmp = (tmp / POOLS(n,2)) / mV%days_per_step
               ! Fourth, if the already committed foliar loss fraction is not big enough, increase it.
               FLUXES(n,9) = min(1d0,FLUXES(n,9) + tmp)
               ! Fifth, calculate the new canopy maintenance demand to balance the budget
               FLUXES(n,55) = FLUXES(n,55) * (1d0-(tmp * mV%days_per_step))
           end if
           ! In all cases the available labile becomes zero
           available_labile = 0d0
       end if
       ...
       ! Estimate wood pool density dependent turnover (fraction / day)
       DIAGS(n,16)  = pars(6) * (POOLS(n,4) / (POOLS(n,4) + pars(36)))

       ! total labile release
       FLUXES(n,8)  = available_labile*(1d0-(1d0-FLUXES(n,16))**mV%days_per_step)/mV%days_per_step
       ! total leaf litter production
       FLUXES(n,10) = POOLS(n,2)*(1d0-(1d0-FLUXES(n,9))**mV%days_per_step)/mV%days_per_step
       ! total wood litter production
       FLUXES(n,11) = POOLS(n,4)*(1d0-(1d0-DIAGS(n,16))**mV%days_per_step)/mV%days_per_step
       ...
       tmp = POOLS(n,8)*(1d0-(1d0-FLUXES(n,2)*pars(35))**mV%days_per_step)*mV%deltat_1(n)
       ! Partition litter turnover between mineralisation and decomposition
       ...
       FLUXES(n,55) = FLUXES(n,55) + FLUXES(n,8)*Rg_fraction ; FLUXES(n,3)  = FLUXES(n,3) + FLUXES(n,55)
       ...
       ! litwood
       POOLS(n+1,8) = POOLS(n,8) + (FLUXES(n,11)-FLUXES(n,30)-FLUXES(n,31))*mV%days_per_step
       ...
       if (met(8,n) > 0d0) then
           ...
           if (C_total > 0d0) then
               ...
               POOLS(n+1,8) = POOLS(n+1,8) + wood_residue
               ! mass balance check
               ! NOTE: this unintentionally forces positive values in the soil surface
               ! water pool too (pool 7). However, as this is also a positive definite pool
               ! it is computationally more efficient to do this than to do pools 1:6 and
               ! line for pool 8 (wood litter)
               ...
               FLUXES(n,34) = (labile_loss-labile_residue) * mV%deltat_1(n)
               FLUXES(n,35) = (foliar_loss-foliar_residue) * mV%deltat_1(n)
               FLUXES(n,36) = (roots_loss-roots_residue) * mV%deltat_1(n)
               FLUXES(n,37) = (wood_loss-wood_residue) * mV%deltat_1(n)
               FLUXES(n,38) = 0d0 ! litter extraction by harvest
               FLUXES(n,39) = 0d0 ! wood litter extraction by harvest
               FLUXES(n,40) = soil_loss_with_roots * mV%deltat_1(n)
               ! Convert harvest related residue generations to daily rate for output
               FLUXES(n,41) = labile_residue * mV%deltat_1(n)
               FLUXES(n,42) = foliar_residue * mV%deltat_1(n)
               FLUXES(n,43) = roots_residue * mV%deltat_1(n)
               FLUXES(n,44) = wood_residue * mV%deltat_1(n)
       ...
       if (met(9,n) > 0d0 .or.(met(8,n) > 0d0 .and. harvest_management > 0)) then
           ...
           if (burnt_area > 0d0) then
               ...
               FLUXES(n,32) = POOLS(n+1,8)*burnt_area*cf(7)/mV%days_per_step ! litterwood
               ...
               FLUXES(n,33) = POOLS(n+1,8)*burnt_area*(1d0-cf(7))*(1d0-rfac(7))/mV%days_per_step ! wood litter into som
               ...
               POOLS(n+1,6) = POOLS(n+1,6) + (FLUXES(n,27) + FLUXES(n,28) + FLUXES(n,33) - FLUXES(n,23)) * mV%days_per_step
               POOLS(n+1,8) = POOLS(n+1,8) - (FLUXES(n,32) + FLUXES(n,33)) * mV%days_per_step
