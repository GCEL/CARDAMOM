...
!  integer:: ncce_lag_remembered
  ...
  double precision:: ncce_lag, ncce_history &
  ...
  double precision, allocatable, dimension(:) :: &
                          ! ncce_history, &!,cgi_history, cmi_history, &
!                                                          tmp_x, &
                                                 Q10_adjustment, &
                                                 Rg_from_labile, &
                                                 Rm_from_labile, &
...
!                         ! NOTE: must be updated any time NUE or avN changes
    ...
    if (.not.allocated(deltat_1)) then
        ...
        allocate(Cwood_labile_release_coef(nodays), Croot_labile_release_coef(nodays), &
                 ...
                 CMI(nodays), NCCE(nodays), rSWP_time(nodays), root_depth_time(nodays))
...
!        cgi_lag_remembered = max(2, nint(21d0/mean_days_per_step))
!        allocate(tmp_x(cgi_lag_remembered) &
!                ,ncce_history(cgi_lag_remembered) &
!                ,cgi_history(cgi_lag_remembered) &
!                ,cmi_history(cgi_lag_remembered))
!        do f = 1, cgi_lag_remembered
...
!        end do
    ...
    SLA = pars(17)**(-1d0)
!    ! Calculate timing components needed for phenology models
!    ncce_lag_remembered = max(2, nint(pars(26)/mean_days_per_step))
!    if (allocated(ncce_history)) deallocate(ncce_history)
!    allocate(ncce_history(ncce_lag_remembered))
    ! Determine the number of time steps we will be averging over
    ncce_lag = max(1d0, dble(nint(pars(26)/mean_days_per_step)))
    ! Load the initial NCCE reference value
...
!    endif
  ...
  subroutine calculate_leaf_dynamics(current_step, deltat, nodays, mean_max_airt, mean_dayl  &
    ...
    ! 4) Based on CMI, CGI and NCCE growth and mortality are determined.
    ...
    ! GSI extended into explicit Canopy Growth Index (CGI) and Canopy Mortality Index (CMI).
    ! CMI and CGI allow for differing phenological sensitivity to drive canopy growth and mortality.
...
!    cmi_ncce = 1d0-max(0d0, NCCE(current_step) / (NCCE(current_step) + cmi_ncce_k50))
    ! Update rolling average NCCE
    NCCE_history = ((NCCE_history * (ncce_lag-1d0)) + NCCE(current_step)) / ncce_lag
    ! Determine the degree of suppression being applied to the potential turnover
    if (NCCE_history > 0d0) then
        cmi_ncce = 1d0-max(0d0, NCCE_history / (NCCE_history+cmi_ncce_k50))
    else
        cmi_ncce = 1d0
    endif

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!    ! Update the NCCE history variable
!    ncce_history(2) = ncce_history(1)
!    ncce_history(1) = NCCE(current_step)
!    ! Determine degree of suppression being applied to the potential turnover
!    if (ncce_history(1) > ncce_history(2)) then
!        cmi_ncce = 0d0
!    else if
!        cmi_ncce = (ncce_history(2) - ncce_history(1)) / ncce_history(2)
!    end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!    ! Update the NCCE history variable
!    do m = 1, ncce_lag-1
!       ncce_history(m+1) = ncce_history(m)
!    end do
!    ncce_history(1) = NCCE(current_step)
!    ! Determine the degree of suppression being applied to potential turnover
!    cmi_ncce = maxval(ncce_history)
!    if (cmi_ncce > 0d0) then
!        cmi_ncce = max(0d0, NCCE(current_step) / cmi_ncce)
!        cmi_ncce = 1d0 - (cmi_ncce / (cmi_ncce+cmi_ncce_k50))
!    else
!        cmi_ncce = 1d0
!    endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!    cmi_ncce = maxval(ncce_history)
!    if (cmi_ncce > 0d0) then
!        cmi_ncce = 1d0-max(0d0, NCCE(current_step) / cmi_ncce)
!    else
!        cmi_ncce = 1d0
!    endif
...
!    cgi_temperature = opt_max_scaling(cgi_temperature_coef_1, cgi_temperature_coef_2 &
!                                         ,cgi_temperature_coef_3, cgi_temperature_coef_4, mean_max_airt)
...
!    cgi_water = logistic_func(rSWP, cgi_water_coef_1, cgi_water_coef_2, 0d0, 1d0)
    ...
    ! CMI is the product of 2 limiting factors for temperature and
    ! water availability that scale between 0 to 1
    ! A larger CMI indicates greater indiction of canopy mortality

    ! Temperature limitation
!    cmi_temperature = opt_max_scaling(cmi_temperature_coef_1, cmi_temperature_coef_2 &
!                                     ,cmi_temperature_coef_3, cmi_temperature_coef_4, leafT)
    ! Water availability limitation
!    cmi_water = logistic_func(wSWP, cmi_water_coef_1, cmi_water_coef_2, 0d0, 1d0)
...
!SHOULD THIS BE A DECLINE RELATIVE TO THE MAX VALUE IN THE LAST 9 MONTHS? I.E. COVERING ONE GROWING SEASON?
!    cmi_ncce = 1d0-max(0d0, NCCE(current_step) / (NCCE(current_step) + cmi_ncce_k50))
! Assume increase in NCCE means no turnover, otherwise allow turnover
!    cmi_ncce = 1d0
!    if (current_step == 1) then
!        cmi_ncce = 0d0
!    else if (NCCE(current_step) > NCCE(current_step-1)) then
!        cmi_ncce = 0d0
!    end if

    !                                               max gradient    point at 50 %
    !cmi_ncce = 1d0-logistic_func(NCCE(current_step), cmi_lab_k50, cmi_ncce_k50, 0d0, 1d0)
    ! Calculate and store the CMI
    !CMI(current_step) = (1d0 - (cmi_temperature*cmi_water))
    CMI(current_step) = 1d0-CGI(current_step)
    ...
    ! If CMI is greater than zero
    else if (CMI(current_step) > 1d-6) then

        ! Determine the potential leaf fall as function of CMI and then
        ! restrict based on function of NCCE (cmi_ncce)
        !leaf_fall = base_leaf_fall + (pot_leaf_fall*CMI(current_step) * cmi_ncce)
        leaf_fall = pot_leaf_fall*CMI(current_step) * cmi_ncce
...
!        endif
!
!        ! Estimate the potential NCCE per gCfoliage, 
!        ! if this is an improvement on the existing canopy then allow turnover
!        if (((tmp/pot_foliage) - Rm_leaf_per_gC) < NCCE(current_step)) leaf_fall = 0d0
