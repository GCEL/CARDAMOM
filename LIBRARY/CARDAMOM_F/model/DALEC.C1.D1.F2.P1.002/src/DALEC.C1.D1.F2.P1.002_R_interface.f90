...
! Subroutine to allow direct interface between DALEC.C1.D1.F2.P1 and the R code
!
...
!endif
     ...
     out_var1(i,1:nodays,27) = FLUXES(1:nodays,30)      ! harvest extracted from labile (gC/m2/day)
     out_var1(i,1:nodays,28) = FLUXES(1:nodays,31)      ! harvest extracted from foliage (gC/m2/day)
     out_var1(i,1:nodays,29) = FLUXES(1:nodays,32)      ! harvest extracted from fine roots (gC/m2/day)
     out_var1(i,1:nodays,30) = FLUXES(1:nodays,33)      ! harvest extracted from wood (gC/m2/day)
     out_var1(i,1:nodays,31) = FLUXES(1:nodays,34)      ! harvest extracted from litter (gC/m2/day)
     out_var1(i,1:nodays,32) = FLUXES(1:nodays,35)      ! harvest extracted from som (gC/m2/day)
     out_var1(i,1:nodays,33) = FLUXES(1:nodays,36)      ! harvest litter / residue from labile (gC/m2/day)
     out_var1(i,1:nodays,34) = FLUXES(1:nodays,37)      ! harvest litter / residue from foliage (gC/m2/day)
     out_var1(i,1:nodays,35) = FLUXES(1:nodays,38)      ! harvest litter / residue from fine roots (gC/m2/day)
     out_var1(i,1:nodays,36) = FLUXES(1:nodays,39)      ! harvest litter / residue from wood (gC/m2/day)
     ! C pools (gC/m2)
     ...
     out_var1(i,1:nodays,43) = DIAGS(1:nodays,1)        ! LAI (m2/m2)
     ! Photosynthesis / C~water coupling related
     out_var1(i,1:nodays,44) = DIAGS(1:nodays,2)        ! ratio of leaf internal to external CO2
     ...
     where (POOLS(1:nodays,1) > 0d0) ! protection against NaN from division by zero
            ...
            tmp = ((FLUXES(1:nodays,8) + &
                    ...
                    FLUXES(1:nodays,30) + FLUXES(1:nodays,36)) / POOLS(1:nodays,1))
     end where
     ...
     where (POOLS(1:nodays,2) > 0d0) ! protection against NaN from division by zero
            ...
            tmp = ((FLUXES(1:nodays,10) + &
                    ...
                    FLUXES(1:nodays,31) + FLUXES(1:nodays,37)) / POOLS(1:nodays,2))
     end where
     ...
     where (POOLS(1:nodays,3) > 0d0) ! protection against NaN from division by zero
            ...
            tmp = ((FLUXES(1:nodays,12) + &
                    ...
                    FLUXES(1:nodays,32) + FLUXES(1:nodays,38)) / POOLS(1:nodays,3))
     end where
     ...
     where (POOLS(1:nodays,4) > 0d0) ! protection against NaN from division by zero
            ...
            tmp = ((FLUXES(1:nodays,11)+ &
                    ...
                    FLUXES(1:nodays,33) + FLUXES(1:nodays,39)) / POOLS(1:nodays,4))
     end where
     ...
     where (POOLS(1:nodays,5) > 0d0) ! protection against NaN from division by zero
            ...
            tmp = ((FLUXES(1:nodays,13) + FLUXES(1:nodays,15) + &
                    ...
                    FLUXES(1:nodays,34)) / POOLS(1:nodays,5))
     end where
     ...
     where (POOLS(1:nodays,6) > 0d0) ! protection against NaN from division by zero
            pool_hak = 0 
            tmp = ((FLUXES(1:nodays,14) + FLUXES(1:nodays,23) + FLUXES(1:nodays,35)) &
                  / POOLS(1:nodays,6))
     end where
