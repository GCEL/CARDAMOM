...
module CARBON_MODEL_MOD
  ...
  subroutine initialize_mv(mV, nodays, nomet, nopars, deltat, soil_frac_sand, soil_frac_clay, met, lat)
    ...
    ! call update_soil_initial_conditions(pars(40), mV)
    ...
    mV%cgi_ncce_lag_step = max(2,nint(21d0/mV%mean_days_per_step))
    allocate(mV%cgi_ncce_lag_days(mV%cgi_ncce_lag_step),mV%cgi_lag_history(mV%cgi_ncce_lag_step),mV%ncce_lag_history(mV%cgi_ncce_lag_step))
    call initialise_cgi(mV%mean_days_per_step,mV%cgi_ncce_lag_step,mV%cgi_ncce_lag_days)
  ...
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,FLUXES,POOLS,DIAGS &
    ...
    ! 3  = autotrophic respiration - maintenance + growth (gC/m2/day)
    ! 4  = leaf maintenance respiration demand from foliar pool (gC/m2/day)
    ...
    ! 16 = CGI labile release factor (fraction/day)
    ! 17 = total ecosystem fire emission - sum(18:23,44) (gC/m2/day)
    ...
    ! 32 = total harvest extracted C - sum(33:39) (gC/m2/day)
    ! 33 = harvest extraction from labile (gC/m2/day)
    ! 34 = harvest extraction from foliage (gC/m2/day)
    ! 35 = harvest extraction from fine roots (gC/m2/day)
    ! 36 = harvest extraction from wood (gC/m2/day)
    ! 37 = harvest extraction from litter (gC/m2/day)
    ! 38 = harvest extraction from wood litter (gC/m2/day)
    ! 39 = harvest extraction from som (gC/m2/day)
    ! 40 = harvest litter residue from labile (gC/m2/day)
    ! 41 = harvest litter residue from foliage (gC/m2/day)
    ! 42 = harvest litter residue from fine roots (gC/m2/day)
    ! 43 = harvest litter residue from wood (gC/m2/day)
    ! 44 = fire emission from wood litter (gC/m2/day)
    ! 45 = fire mortality transfer wood litter->som (gC/m2/day)
    ...
    ! 51 = drainage from surface to 2nd soil layer (kgH2O/m2/day)
    ! 52 = infiltration into top soil layer (kgH2O/m2/day)
    ! 53 = fraction of transpiration from 1st rooting layer (0-1)
    ! 54 = fraction of transpiration from 2nd rooting layer (0-1)
    ! 55 = growth respiration from labile->foliage transfer plus leaf maintenance respiration (gC/m2/day)
    ...
    ! p(3)  = initial canopy CGI value (0-1)
    ...
    ! p(11) = log10 average foliar nitrogen (gN/m2)
    ! p(12) = max labile turnover rate to foliage (fraction/day)
    ! p(13) = fraction of NPP allocated to labile pool (fraction)
    ! p(14) = canopy CGI phenology gradient threshold
    ! p(15) = net canopy carbon export return on new foliage investment (gC/gC)
    ! p(16) = wood litter turnover rate, temperature adjusted (fraction/day)
    ...
    ! p(24) = initial wood litter C pool (gC/m2)
    ...
    ! p(34) = CGI minimum temperature for development (oC)
    ! p(35) = CGI optimum temperature for development (oC)
    ! p(36) = CGI maximum temperature for development (oC)
    ! p(37) = CGI temperature response kurtosis
    ! p(38) = soil water potential at which foliar growth is fully suppressed (MPa)
    ! p(39) = soil water potential at which foliar growth suppression begins (MPa)
    ! p(40) = initial soil water fraction (m3/m3)
    ! p(41) = nitrogen use efficiency (gC/gN/m2/day)
    ! p(42) = leaf nitrogen linked respiration exponential coefficient
    ! p(43) = leaf nitrogen linked respiration intercept
    ! p(44) = net canopy carbon export half saturation for CGI suppression (gC/gCleaf/day)
    ! p(45) = net canopy carbon export gradient 50% value for CGI suppression (gC/gCleaf/day)
    ! p(46) = absolute minimum temperature for leaf growth (oC)
    ! p(47) = temperature range above p(46) at which cold limitation is 50% (oC)
    ! p(48) = initial net canopy carbon export reference value (gC/gCleaf/day)
    ...
    mV%avN = 10d0**pars(11) ! foliar N gN/m2
    mV%NUE = pars(41)       ! Photosynthetic nitrogen use efficiency at optimum temperature (oC)
    ...
    Rm_leaf_baseline = Rm_reich_N(pars(17)/mV%avN,pars(42),pars(43)) * umol_to_gC * seconds_per_day * 2d-3
    ...
    mV%cgi_lag_history = pars(3)
    mV%ncce_lag_history = pars(48)
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       ! (gC/gCleaf/day).
       Rm_leaf_per_gC = Rm_leaf_baseline * Rm_reich_Q10(mV%meant)
       ...
       available_labile = POOLS(n,1) + ((FLUXES(n,5)-FLUXES(n,4)) * mV%days_per_step)
       ! Do plant allocation
       call plant_canopy_phenology(nodays, n, mV%days_per_step,                                  & ! Timing
       ...
       tmp = POOLS(n,8)*(1d0-(1d0-FLUXES(n,2)*pars(16))**mV%days_per_step)*mV%deltat_1(n)
       ! Partition litter turnover between mineralisation and decomposition
       ...
       ! labile pool
       POOLS(n+1,1) = POOLS(n,1) + (FLUXES(n,5)-FLUXES(n,8)-FLUXES(n,55))*mV%days_per_step
       ! foliar pool
       POOLS(n+1,2) = POOLS(n,2) + (FLUXES(n,8)-FLUXES(n,10))*mV%days_per_step
       ! root pool
       ...
       ! som pool
       POOLS(n+1,6) = POOLS(n,6) + (FLUXES(n,15)+FLUXES(n,31)-FLUXES(n,14))*mV%days_per_step
       ! litwood
       ...
       FLUXES(n,49) = mV%runoff          ! soil surface runoff (kgH2O/m2/day)
       FLUXES(n,50) = mV%underflow       ! drainage from bottom of soil column (kgH2O/m2/day)
       FLUXES(n,51) = mV%water_grav_flow(1) ! drainage from the surface soil layer to 2nd (kgH2O/m2/day)
       FLUXES(n,52) = mV%infiltrated(1)  ! top soil surface infiltration by rain (kgH2O/m2/day)
       ...
       FLUXES(n,53) = mV%uptake_fraction(1) ! transpiration fraction extracted from 1st rooting layer (the soil surface)
       FLUXES(n,54) = mV%uptake_fraction(2) ! transpiration fraction extracted from 2nd rooting layer (dynamic 2nd layer)       
       ...
       if (met(8,n) > 0d0) then
           ...
           if (C_total > 0d0) then
               ...
               FLUXES(n,33) = (labile_loss-labile_residue) / mV%days_per_step  ! Labile extraction
               FLUXES(n,34) = (foliar_loss-foliar_residue) / mV%days_per_step  ! foliage extraction
               FLUXES(n,35) = (roots_loss-roots_residue) / mV%days_per_step    ! fine roots extraction
               FLUXES(n,36) = (wood_loss-wood_residue) / mV%days_per_step      ! wood extraction
               FLUXES(n,37) = 0d0 ! litter extraction
               FLUXES(n,38) = 0d0 ! wood litter extraction by harvest               
               FLUXES(n,39) = soil_loss_with_roots / mV%days_per_step          ! som extraction
               ! Convert harvest related residue generations to daily rate for output
               FLUXES(n,40) = labile_residue / mV%days_per_step ! labile residues
               FLUXES(n,41) = foliar_residue / mV%days_per_step ! foliage residues
               FLUXES(n,42) = roots_residue / mV%days_per_step  ! fine roots residues
               FLUXES(n,43) = wood_residue / mV%days_per_step   ! wood residues
       ...
       if (met(9,n) > 0d0 .or.(met(8,n) > 0d0 .and. harvest_management > 0)) then
           ...
           if (burnt_area > 0d0) then
               ...
               FLUXES(n,44) = POOLS(n+1,7)*burnt_area*cf(7)/mV%days_per_step ! litterwood
               ...
               FLUXES(n,45) = POOLS(n+1,7)*burnt_area*(1d0-cf(7))*(1d0-rfac(7))/mV%days_per_step ! wood litter into som
               ...
               POOLS(n+1,6) = POOLS(n+1,6) + (FLUXES(n,27) + FLUXES(n,28) + FLUXES(n,45) - FLUXES(n,23)) * mV%days_per_step
               POOLS(n+1,7) = POOLS(n+1,7) - (FLUXES(n,44) + FLUXES(n,45)) * mV%days_per_step
  ...
  subroutine acm_gpp_stage_1 (mV)
    ...
    ceff = (((mV%avN * mV%lai) / mV%leaf_canopy_light_scaling) * mV%NUE)
    ...
    mV%metabolic_limited_photosynthesis = gC_to_umol*mV%leaf_canopy_light_scaling*ceff*seconds_per_day_1 &
    ...
    use carbon_model_memory
      ...
      type(model_working_variables) :: mV
       ...
       double precision :: tmp, tmp1, & 
                           rescale, lai_orig, gs_orig, scaling_orig, leaf_investment
       ...
       if (mV%leafT > cgi_temperature_coef_5) then  
           leafT_limit = opt_max_scaling(cgi_temperature_coef_1,cgi_temperature_coef_2 &
                                        ,cgi_temperature_coef_3,cgi_temperature_coef_4,mV%leafT)
           ...
           leafT_limit = ((mV%leafT-cgi_temperature_coef_5) / ((mV%leafT-cgi_temperature_coef_5) + cgi_temperature_coef_6)) * leafT_limit                                        
       end if
       ! Specific limitation of hydraulic limitation on leaf growth, wSWP as proxy.
       if (mV%wSWP > cgi_water_coef_1) then  
           leafW_limit = min(1d0,max(0d0,(mV%wSWP - cgi_water_coef_1) / (cgi_water_coef_2-cgi_water_coef_1)))
       end if      
       ...
       if (step < mV%cgi_ncce_lag_step) then
           if (step == 1) then
               mV%cgi_lag_history(2)  = cgi(step)
               mV%ncce_lag_history(2) = ncce_gCgC(step)
               interval = 2
           else
               mV%cgi_lag_history(1:step)  = cgi(1:step)
               mV%ncce_lag_history(1:step) = ncce_gCgC(1:step)
              
               interval = step
           endif
       else
           mV%cgi_lag_history(1:mV%cgi_ncce_lag_step)  = cgi((step-mV%cgi_ncce_lag_step+1):step)
           mV%ncce_lag_history(1:mV%cgi_ncce_lag_step) = ncce_gCgC((step-mV%cgi_ncce_lag_step+1):step)
           interval = mV%cgi_ncce_lag_step
       end if
       ! Now calculate the linear gradients
       cgi_gradient  = linear_model_gradient(mV%cgi_ncce_lag_days(1:interval),mV%cgi_lag_history(1:interval),interval)
       ncce_gradient = linear_model_gradient(mV%cgi_ncce_lag_days(1:interval),mV%ncce_lag_history(1:interval),interval)
       ...
       if (cgi_gradient > cgi_phenology_threshold .and. cgi(step) > vsmall .and. available_labile > 0d0) then
           ...
           if (alloc_leaf_fraction > 0d0) then
               ...
               delta_ncce_gCgC = (acm_gpp_stage_2(mV%stomatal_conductance, mV) * umol_to_gC * mV%dayl_seconds) &
                               - current_gpp
