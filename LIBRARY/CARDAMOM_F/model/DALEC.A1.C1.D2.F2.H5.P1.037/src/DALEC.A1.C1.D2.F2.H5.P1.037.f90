...
! This file contains the source code of DALEC.A1.C1.D2.F2.H5.P1
!
...
!    print*,"carbon_model: "
    ...
    mV%minlwp = pars(33) ! Minimum leaf water potential                 
    mV%iWUE = pars(34)   ! load the inherent water use efficiency   
    ! Rooting parameters
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       ! Cold threshold inhibition on metabolic activity. 
       ! Based on curve estimates for shutdown of photosynthesis, 
       ! but here the temperature used is daily mean air temperature.
       !if (soilT > Vc_minT) then
           tmp = 1d0!((soilT - Vc_minT) / ((soilT - Vc_minT) + Vc_coef))
  ...
  subroutine calculate_stomatal_conductance (mV)
    ...
    if (mV%aerodynamic_conductance > vsmall .and. mV%total_water_flux > vsmall .and. &
        ...
        mV%iWUE_step = mV%iWUE * mV%leaf_canopy_light_scaling ! umolC/mmolH2Ogs/s 
                     !TODO using variable iWUE, since the parameter of the same name is missing in this model
