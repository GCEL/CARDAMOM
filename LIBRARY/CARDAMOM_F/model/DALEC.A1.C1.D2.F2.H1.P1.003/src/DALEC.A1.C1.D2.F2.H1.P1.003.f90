...
! This file contains the source code of DALEC.A1.C1.D2.F2.H1.P1.003
!
...
module CARBON_MODEL_MOD
  ...
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,FLUXES,POOLS,DIAGS &
    ...
    ! p(24) = fraction of wood C as coarse roots (fraction)
    ! p(25) = root biomass for 50% of maximum rooting depth (gBiomass/m2)
    ! p(26) = maximum rooting depth (m)
    ! p(27) = fire resilience factor for non-combusted C (fraction)
    ! p(28) = combustion completeness for foliage (fraction)
    ! p(29) = combustion completeness for non-photosynthetic tissue (fraction)
    ! p(30) = combustion completeness for soil (fraction)
    ! p(31) = combustion completeness for foliage and fine root litter (fraction)
...
!    print*,"carbon_model: "
    ...
    ! Rooting parameters
    mV%root_k = pars(25) ; mV%max_depth = pars(26)
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    mV%root_biomass = mV%fine_root_biomass + max(min_root,POOLS(1,4)*pars(24)*2d0)
    ...
    do n = start, finish
       ...
       mV%root_biomass = mV%fine_root_biomass + max(min_root,POOLS(n,4)*pars(24)*2d0)
       call calculate_Rtot(mV)
...
!    endif
  ...
  pure function arrhenious( a , b , t )
    use carbon_model_memory, only: freeze
