...
module CARBON_MODEL_MOD
  ...
  ! explicit publics
  public :: CARBON_MODEL, initialize_mv
...
!    print*,"carbon_model: "
    ...
    mV%iWUE = pars(46) ! load the inherent water use efficiency
...
!    print*,"carbon_model: done"
  ...
  subroutine acm_gpp_stage_1 (mV)
    ...
    mV%cold_shutdown = dT_Vc_minT / (dT_Vc_minT + Vc_coef)
    airt_adj = mV%cold_shutdown &
             * modified_arrhenious(298.15d0,Ha_Vcmax,Hd_Vcmax_Jmax,dS_Vcmax,leafT_freeze)
    mV%metabolic_limited_photosynthesis = mV%Vcmax_ref * mV%leaf_canopy_light_scaling &
                                     * airt_adj
    ...
    mV%dark_respiration = (mV%lai * Rm_heskel_polynomial(mV%Rm_leaf_const,mV%leafT,mV%cold_shutdown))
    ...
    Jmax = mV%dayl_hours_fraction*2.787095d0*(mV%Vcmax_ref**0.890d0) * mV%leaf_canopy_light_scaling &
  ...
  !----------------------------------------------------------------------
  !
!  double precision function find_gs_WUE(gs_in, mV)
!
!    ! Calculate CO2 limited photosynthesis as a function of metabolic limited
!    ! photosynthesis (pn), atmospheric CO2 concentration and stomatal
!    ! conductance (gs_in). Photosynthesis is calculated twice to allow for
...
!    endif
  ...
  subroutine plant_allocation(nopools,time,                              & 
    ...
    use carbon_model_memory
      ...
      type(model_working_variables) :: mV
       ...
       ! Local variables
       double precision :: rescale, leafT_adj, lai_orig, gs_orig, scaling_orig, leaf_investment, available_labile_1
  ...
  subroutine plant_natural_turnover(nodays,step,time,                       & ! time related
    use carbon_model_memory
                                    ...
                                    cmi,delta_ncce_gCgC,ncce_gradient, mV)        ! CMI, NCCE change for canopy loss, NCCE gradient
      ...
      type(model_working_variables) :: mV
       ...
       if (foliage > 0d0) then
           ...
           if (step < mV%ncce_lag_step) then
               if (step == 1) then
                   mV%ncce_lag_history(2) = ncce_gCgCday(step)
                   interval = 2
               else
                   mV%ncce_lag_history(1:step) = ncce_gCgCday(1:step)             
                   interval = step
               endif
           else
               mV%ncce_lag_history(1:mV%ncce_lag_step) = ncce_gCgCday((step-mV%ncce_lag_step+1):step)
               interval = mV%ncce_lag_step
           end if
           ! Now calculate the linear gradients
           ncce_gradient = linear_model_gradient(mV%ncce_lag_days(1:interval),mV%ncce_lag_history(1:interval),interval)
           ...
           do while (ncce_positive) 
              ...
              mV%lai = lai_orig - ((proposed_fall(i) * time) / lca)
              ...
              if (mV%lai <= 0d0) then
                  ...
                  tmp(i) = (((acm_gpp_stage_2(mV%stomatal_conductance, mV) + mV%dark_respiration) &
                             * umol_to_gC * mV%dayl_seconds) - (mV%dark_respiration * umol_to_gC * seconds_per_day)) &
