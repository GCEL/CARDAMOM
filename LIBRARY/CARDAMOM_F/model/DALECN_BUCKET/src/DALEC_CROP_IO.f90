...
!  
 ...
 subroutine output_data(FLUXES,POOLS, pars, DS_array, treatment_num)
    ...
    ! Arguments
    integer, intent(in) :: treatment_num
    ...
    double precision, dimension(:), intent(in) :: pars  ! vector of ecosystem fluxes

    double precision, dimension(:), intent(in) :: DS_array  ! vector of ecosystem fluxes
    ...
    ! write FLUXES -----------------------
    open(unit=daily_flux_file, file = (trim(output_dir)//'FLUXES_'//filename//'.csv'), status="unknown")
    ...
    ! write POOLS ------------------------
    open(unit=daily_pools_file, file = (trim(output_dir)//'POOLS_'//filename//'.csv'), status="unknown")
    

    write(daily_pools_file,'(6(A10,","))') "DS", "LAI","stock_foliage","stock_stem","stock_root", "stock_storage"
    write(daily_pools_file,'(6(F16.4,","))') (DS_array(p), (POOLS(p,2)/pars(17)), POOLS(p,2), POOLS(p,4), POOLS(p,3), POOLS(p,9),  p = 1,730)
