program MAIN_DALEC
  ...
  ! Read veg parameter data *csv
  if ( .not. allocated( pars ) )  allocate ( pars( num_pars ) ) ! Allocate LAI
  ...
  run = 5
     ...
     call CARBON_MODEL_CROP(start,finish,met_data,pars,deltat,num_days,lat,lai,NEE,FLUXES,POOLS &
                              ...
                              ,LRLV,DS_LRRT,LRRT, DS_array, leaf_mass(run,:))  



     
     call output_data(FLUXES,POOLS, pars, DS_array, run)
