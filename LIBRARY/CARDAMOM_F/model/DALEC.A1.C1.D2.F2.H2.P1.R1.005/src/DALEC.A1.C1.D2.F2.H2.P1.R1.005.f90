...
! This file contains the source code of DALEC.A1.C1.D2.F2.H2.P1.R1.005
!
...
module CARBON_MODEL_MOD
  ...
  subroutine destroy_mv(mV)
    ...
    integer:: n
        ! allocate variables dimension which are fixed per site only the once
        deallocate(mV%deltat_1,mV%daylength_hours,mV%daylength_seconds, &
                   mV%daylength_seconds_1,mV%rainfall_time,mV%airt_zero_fraction_time)
  end subroutine
  !
  !--------------------------------------------------------------------
  !
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,FLUXES,POOLS,DIAGS &
    ...
    ! Evergreen Analytical - ACMv2 - BUCKET (DALEC.A1.C1.D2.F2.H2.P1.R1.005) model.
    ...
    ! 50 = growth respiration from labile->foliage transfer (gC/m2/day)
    ! 51 = infiltration into middle soil layer (kgH2O/m2/day)
    ! 52 = infiltration into bottom soil layer (kgH2O/m2/day)
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       ! labile pool
       POOLS(n+1,1) = POOLS(n,1) + (FLUXES(n,5)-FLUXES(n,8)-FLUXES(n,50))*mV%days_per_step
       ! foliar pool
       ...
       FLUXES(n,51) = mV%infiltrated(2)  ! middle soil surface infiltration by rain (kgH2O/m2/day)
       FLUXES(n,52) = mV%infiltrated(3)  ! bottom soil surface infiltration by rain (kgH2O/m2/day)
