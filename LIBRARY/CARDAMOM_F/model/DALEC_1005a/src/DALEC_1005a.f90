...
! This file contains the source code of DALEC_1005a
!
...
module CARBON_MODEL_MOD
  ...
  subroutine initialize_mv(mV, nodays, nomet, nopars, deltat, soil_frac_sand, soil_frac_clay, met, lat)
    ...
    ! Not used by this model, but required for code consistency
  ...
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,FLUXES,POOLS,DIAGS &
    ...
    ! Evergreen Analytical (DALEC_CDEA_BUCKET; 1005a; DALEC2) model.
    ...
    ! This version implements the JPL created water cycle model.
    ! This model uses a plant available and unavailable water pools.
    ! Evapotranspiration is assumed to be a proportion of GPP following uWUE (Boese et al., 2017)
    ! while a non-linear response function determines water drainage.
    ! Ref: Yang et al., (2021, submitted to GMDD)
    ...
    ! Version history:
    ! 1) Bug identified in calculation of FLUXES(n,2) that resulted in biased temperature contribution to function
    ! 2) Inconsistency with paper description identified in overflow calculation for plant unavailable water. A *0.25 replaced with *0.5
    ! 3) FLX 9, 16 previously driven by met(1,n) but this leads to allocation timing varying between years
    !    as trigometric functions scale between 1-360 not 365. Driving with doy enforced consistent behaviour.
    ! 4) Mass balance checks on PAW and PUW (POOLS7,8) to prevent negative values.
    !    Negative values cause NaN which cause the model to fail UoE CARDAMOM sanity checks. In JPL framework EDCs reject thse.
    ...
    ! 7th Precipitation (kgH2O/m2/s)
    ! 8th Fraction of biomass removed
    ! 9th Burned fraction
    ! 10th 21 day mean daily maximum temperature (C)
    ! 11th 21 day mean day length (seconds)
    ! 12th 21 day mean daily VPD (Pa)
    ! 13th Biomass removal management type
    ! 14th Mean temperature (C)
    ! 15th Mean wind speed (m/s)
    ! 16th VPD (Pa)
    ...
    ! 29 = evapotranspiration - uWUE-based (kgH2O/m2/day)
    ! 30 = plant available water runoff loss from system (kgH2O/m2/day)
    ! 31 = drainage from plant available to plant unavailable water pool (kgH2O/m2/day)
    ! 32 = plant unavailable water runoff loss from system (kgH2O/m2/day)
    ...
    ! 38 = harvest extraction from som (gC/m2/day)
    ! 39 = harvest litter residue from labile (gC/m2/day)
    ! 40 = harvest litter residue from foliage (gC/m2/day)
    ! 41 = harvest litter residue from fine roots (gC/m2/day)
    ! 42 = harvest litter residue from wood (gC/m2/day)
    ! 43 = total harvest extracted C - sum(33:38) (gC/m2/day)
    ...
    ! p(24) = underlying water use efficiency uWUE (gC/kgH2O per hPa)
    ! p(25) = inverse second-order runoff constant for plant available water pool (mm day)
    ! p(26) = plant available water wilting point (mm)
    ! p(27) = initial plant available water pool (mm)
    ! p(28) = combustion completeness for foliage (fraction)
    ! p(29) = combustion completeness for roots+wood (fraction)
    ! p(30) = combustion completeness for soil (fraction)
    ! p(31) = fire resilience factor for non-combusted C (fraction)
    ! p(32) = labile pool lifespan (yr)
    ! p(33) = moisture response coefficient for heterotrophic activity
    ! p(34) = fraction of plant available water drainage to plant unavailable pool (fraction)
    ! p(35) = inverse second-order runoff constant for plant unavailable water pool (mm day)
    ! p(36) = initial plant unavailable water pool (mm)
    ! p(37) = radiation coefficient modifying uWUE (kgH2O/m2/MJ)
    ! p(38) = combustion completeness for foliage and fine root litter (fraction)
    ...
    if (start == 1) then
       ...
       ! Initial water pool (kgH2O/m2) conditions
       POOLS(1,7) = pars(27) ! plant available water
       POOLS(1,8) = pars(36) ! plant unavailable water
    endif
    ...
    ! fall durations
    wf = pars(16)*sqrt(2d0) * 0.5d0 ! foliage
    wl = pars(14)*sqrt(2d0) * 0.5d0 ! labile
    ! Magnitude coefficient
    ff = (log(pars(5))-log(pars(5)-1d0)) * 0.5d0   ! foliage
    fl = (log(pars(32))-log(pars(32)-1d0)) * 0.5d0 ! labile

    ! Offset for labile and leaf turnovers
    osf = ospolynomial(pars(5),wf)
    osl = ospolynomial(pars(32),wl)
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
      ...
      ! GPP (gC.m-2.day-1), apply linear correction based on plant available water and wilting point (kgH2O/m2)
      FLUXES(n,1) = acm(gpppars,constants, mV%ci) ; DIAGS(n,2) = mV%ci / met(5,n)
      ! Heterotrophic activity coefficent, a function of an exponential temperature response
      ...
      FLUXES(n,2) = exp(pars(10)*((0.5d0*(met(3,n)+met(2,n)))-mean_airt)) &
                  * ( ((((met(7,n)*86400d0*mean_precip_inv))-1d0)*pars(33)) + 1d0)
      ...
      ! litter to som
      FLUXES(n,15) = POOLS(n,5)*(1d0-(1d0-FLUXES(n,2)*pars(1))**deltat(n))/deltat(n)

      !
      ! Estimate evapotranspiration (kgH2O/m2/day) and water balance
      !

      ! Estimate ET as a function of GPP, SW radiation and underlying Water Use Efficiency (uWUE; gC/kgH2O)
      ! following Boese et al. (2017). Note *1d-2 converts VPD Pa -> hPa
      FLUXES(n,29) = ((FLUXES(n,1) * sqrt(met(16,n)*1d-2)) / pars(24) ) + (pars(37)*met(4,n))

      ! Estimate total runoff for plant available water, this will be partitioned between loss and PUW below
      FLUXES(n,30) = (POOLS(n,7)**2d0 / pars(25)) / deltat(n)
      ! Now partition between runoff to plant unavailable then adjust the total to loss from system
      FLUXES(n,31) = FLUXES(n,30)*pars(34) ; FLUXES(n,30) = FLUXES(n,30) * (1d0-pars(34))
      ! Estimate the plant unavailable water runoff
      FLUXES(n,32) = (POOLS(n,8)**2d0 / pars(35)) / deltat(n)

      ! Determine whether current water pool is large enough that unrealistically high losses will occur
      if (POOLS(n,7) > pars(25)*0.5d0) then
          ! Estimate total runoff for plant available water, this will be partitioned between loss and PUW below
          FLUXES(n,30) = (POOLS(n,7) - (pars(25)*0.5d0)) / deltat(n)
          ! Now partition between runoff to plant unavailable then adjust the total to loss from system
          FLUXES(n,31) = FLUXES(n,30)*pars(34) ; FLUXES(n,30) = FLUXES(n,30) * (1d0-pars(34))
      end if
      ! Determine whether current water pool is large enough that unrealistically high losses will occur
      if (POOLS(n,8) > pars(35)*0.5d0) then
          FLUXES(n,32) = (POOLS(n,8)-(pars(35)*0.5d0)) / deltat(n)
      end if

      ! Update plant available water pool, *86400 scale kgH2O/m2/s -> kgH2O/m2/day
      ! NOTE: unsatisfactory hack of hard limit to prevent negative water stocks but without updating fluxes
      POOLS(n+1,7) = max(0d0,POOLS(n,7) + ((met(7,n)*86400d0) - FLUXES(n,29) - FLUXES(n,30) - FLUXES(n,31))*deltat(n))
      ! Update plant unavailable water
      ! NOTE: unsatisfactory hack of hard limit to prevent negative water stocks but without updating fluxes
      POOLS(n+1,8) = max(0d0,POOLS(n,8) + (FLUXES(n,31) - FLUXES(n,32))*deltat(n))

      !
      ! Update pools C based on natural processes
      !
