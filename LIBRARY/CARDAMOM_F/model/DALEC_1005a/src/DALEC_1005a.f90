...
implicit none
  ...
  ! This version of DALEC (consistent with JPL version 1005 but with bug corrections (hence the 'a'))
  ! is derived from the following primary references:
...
!
  subroutine CARBON_MODEL(start, finish, met, pars, deltat, nodays, lat, lai, NEE, FLUXES, POOLS &
    ...
    ! 2) Inconsistency with paper desription identified in overflow calculation for plant unavailable water. A*0.25 replaced with*0.5
    ...
    if (maxval(met(8, :)) > 0d0 .or. maxval(met(9, :)) > 0d0) then
        ...
        cf(2) = pars(28)  ! foliage
        ! Assign combustion completeness to non-photosynthetic
        cf(1) = pars(29); cf(3) = pars(29); cf(4) = pars(29)
        cf(6) = pars(30)  ! soil
    ...
    do n = start, finish
      ...
      FLUXES(n, 1) = acm(gpppars, constants) * min(POOLS(n, 7)/pars(26), 1d0); CiCa_time(n) = ci/met(5, n)
      ...
      FLUXES(n, 2) = exp(pars(10)*((0.5d0*(met(3, n)+met(2, n)))-mean_airt)) &
                  * ( ((((met(7, n)*86400d0)/mean_precip)-1d0)*pars(33)) + 1d0)
      ...
      FLUXES(n, 29) = ((GPP(n) * sqrt(met(16, n)*1d-2)) / pars(24) ) + (pars(37)*met(4, n))
