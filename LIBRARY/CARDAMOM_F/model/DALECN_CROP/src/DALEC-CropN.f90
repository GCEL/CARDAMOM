...
double precision ::           ts_length, & ! time step length in hours
  ...
  double precision, parameter :: LAICR = 4.5
  ! allocation to storage organ relative to GPP 
  double precision, parameter :: rel_gso_max = 0.29
...
!
  subroutine CARBON_MODEL_CROP(start,finish,met,pars,deltat,nodays,lat,lai,NEE,FLUXES,POOLS &
    ...
    double precision, dimension(nodays), intent(inout) :: lai & ! leaf area index
											   ...
											   ,LNA   ! Leaf nitrogen per area (LNA)
	...
	! p(36) ! Slope of nitrogen dilution linear fit for calculating LNA
	! p(37) ! Intercept of nitrogen dilution linear fit for calculating LNA
    ...
    do n = start, finish
      ...
      if (DS .lt. 0.469) then                           ! DS < 0.15 corresponds to the growth stage at beginning of the UK reccommended period of      
      ...
      else if (DS .ge. 0.469 .and. DS .le. 1.293) then                      
         gpppars(4) = (slope_n*POOLS(n,2))+intercept_n ! NOTE: The slope_n parameter can be included in the MDF optimisation. The value for this parameter has also been observed to be close to 
	  else
		 gpppars(4) = 1.25
      end if  
	  ! ------------------------------------------------------ 
