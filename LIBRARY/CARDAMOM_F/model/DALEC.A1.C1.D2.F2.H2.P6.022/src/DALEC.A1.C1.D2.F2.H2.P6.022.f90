...
! This file contains the source code of DALEC.A1.C1.D2.F2.H2.P6
!
...
module CARBON_MODEL_MOD
  ...
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,FLUXES,POOLS,DIAGS &
    ...
    ! p(33) = half saturation coefficient for self-thinning suppression on wood turnover (gC/m2)
    ! p(34) = labile pool lifespan (yr)
...
!    print*,"carbon_model: "
    ...
    fl = (log(pars(34))-log(pars(34)-1d0)) * 0.5d0
    ! offset for labile and leaf turnovers
    osf = ospolynomial(pars(5),wf)
    osl = ospolynomial(pars(34),wl)
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       DIAGS(n,16)  = pars(6) * (POOLS(n,4) / (POOLS(n,4) + pars(33)))
