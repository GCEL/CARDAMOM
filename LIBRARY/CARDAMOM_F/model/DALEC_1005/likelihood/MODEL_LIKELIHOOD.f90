...
! of DALEC_1005 as a function of observations and ecological dynamical constraints.
!
...
module model_likelihood_module
  ...
  subroutine find_edc_initial_values
    ...
    double precision :: PEDC, PEDC_prev, ML_prior, P_target
...
!    end do ! parameter loop
    ...
    if (.not. restart_flag) then
        ...
        do while (PEDC < 0d0)
           ...
           ! Use the best parameter set from that loop as the initial for the next
...
!    ML_obs_out = -0.5d0*(tot_exp*10d0)*DATAin%EDC
  ...
  subroutine sub_model_likelihood(PARS,ML_obs_out,ML_prior_out)
    ...
    ! run the dalec model
    ! NOTE this must remain here for compliance with no EDC runs
    call carbon_model(1,DATAin%nodays,DATAin%MET,PARS,DATAin%deltat &
                     ,DATAin%nodays,DATAin%LAT,DATAin%M_LAI,DATAin%M_NEE &
                     ,DATAin%M_FLUXES,DATAin%M_POOLS,DATAin%nopars &
                     ,DATAin%nomet,DATAin%nopools,DATAin%nofluxes  &
...
!print*,"sanity_check: carbon_model done 2"
  ...
  subroutine assess_EDC1(PARS, npars, meantemp, meanrad, EDC1)
    ...
    ! Subsequently modified to be consistent with DALEC 1005 as described in
    ! Yang et al., (submitted to GMDD)
    ...
    ! NOTE: This is in error due to pars(9) now intended to be applied ta mean temperature.
    !       Error maintainted for consistency with 1005 JPL model
    ...
    ! Combustion completeness for foliage should be greater than non-photosynthetic tissues
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(28) < pars(30)) then
  ...
  subroutine assess_EDC2(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    double precision :: sum_precip, sum_precip_yr1, sum_precip_yr2 &
...
!        etol = 0.1d0
        ...
        ! Estimate total precipitation (kgH2O/m2)
        sum_precip = sum(met(7,:)*86400d0*deltat)
        ...
        ! Estimate total precipitation (kgH2O/m2)
        sum_precip = sum(met(7,io_start:io_finish)*86400d0*deltat(io_start:io_finish))
        sum_precip_yr1 = sum(met(7,1:steps_per_year)*86400d0*deltat(1:steps_per_year))
        sum_precip_yr2 = sum(met(7,(steps_per_year+1):(steps_per_year*2))*86400d0*deltat((steps_per_year+1):(steps_per_year*2)))
    ...
    Fin_yr1(7)  = sum_precip_yr1
    Fout_yr1(7) = FT_yr1(29)+FT_yr1(30)+FT_yr1(31)
    Fin_yr2(7)  = sum_precip_yr2
    Fout_yr2(7) = FT_yr2(29)+FT_yr2(30)+FT_yr2(31)
    ! plant unavailable water
    Fin(8)  = FT(31)
    Fout(8) = FT(32)
    Fin_yr1(8)  = FT_yr1(31)
    Fout_yr1(8) = FT_yr1(32)
    Fin_yr2(8)  = FT_yr2(31)
    Fout_yr2(8) = FT_yr2(32)
...
!               if (abs(log(Fin_yr1(n)/Fout_yr1(n)) - log(Fin_yr2(n)/Fout_yr2(n))) > etol) then
    ...
    end if ! old_edcs or not

    ! Ensure that wilting point is at or below the mean plant available water
    if ((EDC2 == 1 .or. DIAG == 1) .and. (pars(26) > mean_pools(7)) ) then
    ...
    if (EDC2 == 1 .or. DIAG == 1) then
       ...
       do while (n <= nofluxes .and. (EDC2 == 1 .or. DIAG == 1))
          ...
          do while (nn <= (nodays+1) .and. PEDC == 1)
             ! now check conditions
             if (M_FLUXES(nn,n) /= M_FLUXES(nn,n) .or. abs(M_FLUXES(nn,n)) == abs(log(infi))) then
                 EDC2 = 0d0 ; PEDC = 0 ; EDCD%PASSFAIL(35+nopools+n) = 0
             end if ! less than zero and is NaN condition
          nn = nn + 1
          end do ! nn < nodays .and. PEDC == 1
          n = n + 1
