...
!    if (sum(EDCD%PASSFAIL) == 100) stop
  ...
  subroutine EDC1_GSI(PARS, npars, meantemp, meanrad, EDC1)
    ...
    ! nitrogen in it)
    if ((EDC1 == 1 .or. DIAG == 1) .and. ( (pars(17)/(10.0**pars(11))) > pars(41) ) ) then
    ...
    if ((EDC1 == 1 .or. DIAG == 1) .and. ((pars(22)/pars(39)) > pars(41)*2.0) ) then
  ...
  subroutine EDC2_GSI(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    use cardamom_structures, only: DATAin
    use CARBON_MODEL_MOD, only: flab_to_ra, &
...
!    flit = (sum(M_FLUXES(1:nodays,10)+M_FLUXES(1:nodays,12)+M_FLUXES(1:nodays,19))) * sumgpp
...
!               / sum(M_FLUXES(1:nodays,10))
    ...
    ! yrs (95CI = 25yr)
    if ((EDC2 == 1 .or. DIAG == 1) .and. (1/(torlabN*365.25)) > 1.1 ) then
    ...
    ! loop all pools except for 1 and 10
    if (EDC2 == 1 .or. DIAG == 1) then
        do no_pool = 1, nopools
           if (no_pool /= 1 .and. no_pool == /2 .and. no_pool /= 10) then
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. (pars(43) > (1200.0 / (pars(17)/(10.0**pars(11))))) ) then
       EDC2=0 ; EDCD%PASSFAIL(9)=0
    end if
