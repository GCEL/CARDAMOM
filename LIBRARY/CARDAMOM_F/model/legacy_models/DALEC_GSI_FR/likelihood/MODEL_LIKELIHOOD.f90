...
!    torfol=sum((1d0-M_FLUXES(1:nodays,18))*pars(5))/nodays ! GSI only

    torlab = 0d0
    do n = 1, nodays
       ! determines whether any flux actually occurred, i.e. whether or not this
       ! flux was determined to be not needed
       if (M_FLUXES(n,8) > 0d0) then
           torlab = torlab + ((M_FLUXES(n,18))*pars(12))
       endif
    end do
    torlab = torlab / nodays

!    torlab = 0. ; torfol = 0.
!    do n = 2, nodays
!       if (M_FLUXES(n,18) > M_FLUXES(n-1,18)) then
!          ! things have improves to labile turnover
!          torlab = torlab + pars(12) ! labile
!          torfol = torfol + 0.       ! foliar
!       elseif (M_FLUXES(n,18) < M_FLUXES(n-1,18)) then
!          ! things have gotten worst, foliar turnover
!          torlab = torlab + 0.       ! labile
!          torfol = torfol + pars(5)  ! foliar
!       else
!          ! must be the same so we do nothing
!          torlab = torlab + 0. ; torfol = torfol + 0.
!       endif ! GSI condition
!    enddo
!    torlab=torlab / nodays
...
!    meangpp=sum(M_GPP(1:nodays))/real(nodays)
    ...
    sumnpp=1/sum(M_GPP(1:nodays)*(1d0-pars(2)))
...
!    flit=(froot+flab+ffol)
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. (torfol < pars(6) .or. (1/(torfol*365.25)) > 8d0 .or. (1/(torfol*365.25)) < 0.15) ) then
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. (1/(torlab*365.25)) > 25d0 ) then
...
!        if (abs(-log(2d0)/decay_coef) < (365.25*real(no_years_adjust)) .and. decay_coef < 0d0 ) then
...
!        decay_coef=expdecay2(M_POOLS,6,deltat,nopools,nodays+1)
    ...
    ! now check
    if ((EDC2 == 1 .or. DIAG == 1) .and. ( (sum(mean_annual_pools)/real(no_years)) > 1200d0 ) ) then
    ...
    ! system
    EQF=log(5) ! 10.0 = order magnitude; 2 = double and half (not accurate any longer)

    ! labile input / output ratio
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_lab)) > log(EQF)) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
    ! foliage input / output ratio
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_fol)) > log(EQF)) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
    ! roots input / output ratio
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_root)) > log(EQF)) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
    ! wood input / output ratio
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_wood)) > log(EQF)) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
    ! litter input / output ratio
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_lit)) > log(EQF)) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
    ! som input / output ratio
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_som)) > log(EQF)) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
    ...
    ! so for generality these yield curves will be used here in broadest sense
     ...
     if (EDC2 == 1 .or. DIAG == 1) then
         ...
         if (DATAin%age /= -9999 .or. maxval(met(8,:)) > 0.99 ) then
            ...
            DATAin%age=hold
            if (target_living_C(2) > 500.0) then
                if (model_living_C < (target_living_C(1)) .or. model_living_C > (target_living_C(2))) then
                    EDC2=0 ; EDCD%PASSFAIL(13)=0
                end if
           ...
           do n = 1, nopools
              ...
              if ((EDC2 == 1 .or. DIAG == 1) .and. ((mean_annual_pools(no_years)/mean_annual_pools(1)) > (1d0+G*real(no_years)))) then
                 EDC2=0 ; EDCD%PASSFAIL(13)=0
              endif
           end do ! pool loop
...
!TLS N_ACM           if (n == 17 .or. n == 11) then
           if (n == 17) then
               likelihood_p=likelihood_p-0.5*((pars(n)-parpriors(n))/parpriorunc(n))**2d0
           else
               likelihood_p=likelihood_p-0.5*(log(pars(n)/parpriors(n))/log(parpriorunc(n)))**2d0
           end if
       end if
    end do
