...
!    endif
    ...
    ! nitrogen in it)
    if ((EDC1 == 1 .or. DIAG == 1) .and. ( (pars(22)/pars(55)) < pars(48) ) ) then
       EDC1=0 ; EDCD%PASSFAIL(10)=0
    endif

    ! C:N ratio of the foliar should be smaller than the wood (i.e. have more
    ! nitrogen in it)
    if ((EDC1 == 1 .or. DIAG == 1) .and. ( (pars(17)/(10.0**pars(11))) > pars(47) ) ) then
       EDC1=0 ; EDCD%PASSFAIL(12)=0
    endif

    ! similarly the C:N ratio of the litter pool cannot be greater than is
    ! largest input (i.e. wood). Here we will add some space to be generous and
    ! say within twice the wood
    if ((EDC1 == 1 .or. DIAG == 1) .and. ((pars(22)/pars(55)) > pars(47)*2.0) ) then
       EDC1=0 ; EDCD%PASSFAIL(18)=0
    endif

    ! initial labile pools cannot be greater than fraction allowed in initial
    ! wood
    if ((EDC1 == 1 .or. DIAG == 1) .and. ( (pars(18)+pars(39)+pars(40)) > (pars(21)*pars(45)) ) ) then
  ...
  subroutine EDC2_GSI(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    use cardamom_structures, only: DATAin
    use CARBON_MODEL_MOD, only: flab_to_ra,rlab_to_ra,wlab_to_ra, &
    ...
    integer :: n, DIAG, no_years, y, PEDC, nn, num_EDC, max_location(1) &
              ,i, exp_adjust, no_years_adjust,no_pool, disturb_year, replant_year
    double precision :: mean_pools(nopools), G, decay_coef, meangpp, EQF5,EQF2 &
                       ,sumgpp, sumnpp, model_living_C, target_living_C(2),hold
    double precision, dimension(nodays) :: mean_ratio,resid_fol,resid_lab,resid_root,resid_wood,resid_lit
    ...
    resid_root(1:nodays) = M_FLUXES(1:nodays,12)/M_POOLS(1:nodays,3)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    where ( M_POOLS(1:nodays,3) == 0 )
    ...
    hak = 0 ; resid_root = 0d0
    resid_root(1:nodays) = (M_FLUXES(1:nodays,6)+rlab_to_ra(1:nodays))/M_POOLS(1:nodays,7)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    where ( M_POOLS(1:nodays,7) == 0 )
           hak = 1 ; resid_root(1:nodays) = 0d0
    end where
    torrootlab = sum(resid_root) / dble(nodays-sum(hak))

    ! calculate labile wood residence
    hak = 0 ; resid_wood = 0d0
    resid_wood(1:nodays) = (M_FLUXES(1:nodays,7)+wlab_to_ra(1:nodays))/M_POOLS(1:nodays,8)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    ...
    hak = 0 ; resid_lab = 0d0
    resid_lab(1:nodays) = (M_FLUXES(1:nodays,8)+flab_to_ra(1:nodays))/M_POOLS(1:nodays,1)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    ...
    hak = 0 ; resid_lab = 0d0
    resid_lab(1:nodays) = (M_FLUXES(1:nodays,8)/(pars(17)/(10.0**pars(11))))/M_POOLS(1:nodays,10)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    where ( M_POOLS(1:nodays,10) == 0 )
           hak = 1 ; resid_lab(1:nodays) = 0d0
    end where
    torlabN = sum(resid_lab) / dble(nodays-sum(hak))

    hak = 0 ; resid_lit = 0d0
    resid_lit(1:nodays) = M_FLUXES(1:nodays,13)/M_POOLS(1:nodays,5)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    where ( M_POOLS(1:nodays,5) == 0 )
           hak = 1 ; resid_lit(1:nodays) = 0d0
    end where
    torlit = sum(resid_lit) / dble(nodays-sum(hak))

    hak = 0 ; resid_lit = 0d0
    resid_lit(1:nodays) = M_FLUXES(1:nodays,15)/M_POOLS(1:nodays,5)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    where ( M_POOLS(1:nodays,5) == 0 )
           hak = 1 ; resid_lit(1:nodays) = 0d0
    end where
    decomplit = sum(resid_lit) / dble(nodays-sum(hak))
...
!    flit = (sum(M_FLUXES(1:nodays,10)+M_FLUXES(1:nodays,11)+M_FLUXES(1:nodays,12))) * sumgpp
!    fsom = sum(M_FLUXES(1:nodays,15)) * sumgpp

    ! NPP allocations; note that because of possible labile accumulation this
    ! might not be equal to 1
    sumnpp=1/sum(M_GPP(1:nodays)*(1.0-fauto))
...
!    in_out_lab  = sum(M_FLUXES(1:nodays,5)) / sum(M_FLUXES(1:nodays,8)+flab_to_ra(1:nodays))
    in_out_labw = sum(npp_to_wood(1:nodays)) / sum(M_FLUXES(1:nodays,7)+wlab_to_ra(1:nodays))
    in_out_labr = sum(npp_to_roots(1:nodays)) / sum(M_FLUXES(1:nodays,6)+rlab_to_ra(1:nodays))
!    in_out_fol  = sum(M_FLUXES(1:nodays,8)) / sum(M_FLUXES(1:nodays,10))
    in_out_root = sum(M_FLUXES(1:nodays,6)) / sum(M_FLUXES(1:nodays,12))
    in_out_wood = sum(M_FLUXES(1:nodays,7)) / sum(M_FLUXES(1:nodays,11))
    in_out_lit  = sum(M_FLUXES(1:nodays,10)+M_FLUXES(1:nodays,19)+M_FLUXES(1:nodays,12)+disturbance_residue_to_litter(1:nodays)) &
                / sum(M_FLUXES(1:nodays,13)+M_FLUXES(1:nodays,15))
    in_out_cwd  = sum(M_FLUXES(1:nodays,11)+disturbance_residue_to_cwd(1:nodays)) / sum(M_FLUXES(1:nodays,19))
    ...
    ! safely constrain foliar(8), root(6) and wood(7) growth the < 20 gCm-2.day-1
    if ((EDC2 == 1 .or. DIAG == 1) .and. (maxval(M_FLUXES(1:nodays,8)) > 20.0) ) then
       EDC2=0 ; EDCD%PASSFAIL(25)=0
    endif
    if ((EDC2 == 1 .or. DIAG == 1) .and. (maxval(M_FLUXES(1:nodays,6)) > 20.0) ) then
    ...
    ! turnover of roots should be faster than that of wood
    if ((EDC2 == 1 .or. DIAG == 1) .and. torroot < pars(6)) then
       EDC2=0 ; EDCD%PASSFAIL(7)=0
    endif

    ! Mineralisation of litter than turnover of som
    if ((EDC2 == 1 .or. DIAG == 1) .and. ((pars(9)*exp(pars(10)*meantemp)) > torlit)) then
        EDC2=0 ; EDCD%PASSFAIL(1)=0
    endif

    ! Litter decomposition to som greater than som to atm rate
    if ((EDC2 == 1 .or. DIAG == 1) .and. (decomplit < (pars(9)*exp(pars(10)*meantemp)))) then
        EDC2=0 ; EDCD%PASSFAIL(2)=0
    endif
    ...
    ! Richardson et al (2015) New Phytologist, Clab residence time = 11 +/- 7.4
    ! yrs (95CI = 25yr)
    if ((EDC2 == 1 .or. DIAG == 1) .and. (1/(torlabN*365.25)) > 0.9 ) then
        EDC2=0 ; EDCD%PASSFAIL(22)=0
    endif

    ! Richardson et al (2015) New Phytologist, Clab residence time = 11 +/- 7.4
    ! yrs (95CI = 25yr)
    if ((EDC2 == 1 .or. DIAG == 1) .and. (1/(torrootlab*365.25)) > 25.0 ) then
...
!        mean_ratio=(M_POOLS(1:nodays,1)+M_POOLS(1:nodays,7)+M_POOLS(1:nodays,8))/M_POOLS(1:nodays,4) ; hak = 0
!        where ( M_POOLS(1:nodays,4) == 0 )
!               hak = 1 ; mean_ratio(1:nodays) = 0d0
!        end where
!        if ((sum(mean_ratio)/dble(nodays-sum(hak))) > 0.125) then
!!print*,"mean_ratio",(sum(mean_ratio)/dble(nodays-sum(hak)))
...
!    endif
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. ( ((mean_pools(3)/mean_pools(2)) < 0.1) .or. &
                                           ((mean_pools(3)/mean_pools(2)) > 4.0) ) ) then
    ...
    ! loop all pools
    if (EDC2 == 1 .or. DIAG == 1) then
        do no_pool = 1, nopools ! 9
           if (no_pool /= 1 .and. no_pool /= 7 .and. no_pool /= 8 .and. no_pool /= 10) then
              decay_coef=expdecay2(M_POOLS(exp_adjust:(nodays+1),:),no_pool,deltat(exp_adjust:nodays),nopools,(nodays+1-exp_adjust+1))
    ...
    ! by the same token we can assume that the Clab:Nlab means that the labile
    ! pool must be smaller than this total value
    if ((EDC2 == 1 .or. DIAG == 1) .and. (pars(54) > (1200.0 / (pars(17)/(10.0**pars(11))))) ) then
       EDC2=0 ; EDCD%PASSFAIL(9)=0
    end if
    ...
    ! foliar labile input / output ratio
!    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_lab)) > EQF5) then
!       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
!    endif
    ! wood labile input / output ratio
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_labw)) > EQF5) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
    ! root labile input / output ratio
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_labr)) > EQF5) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
...
!    endif
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(log(in_out_cwd)) > EQF5) then
       EDC2 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
