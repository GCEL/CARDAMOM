...
!    if (sum(EDCD%PASSFAIL) == 100) stop
  ...
  subroutine EDC1_GSI(PARS, npars, meantemp, meanrad, EDC1)
    ...
    ! Turnover of litter and cwd faster than turnover of som
  ...
  subroutine EDC2_GSI(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    hak = 0 ; resid_root = 0d0
    resid_root(1:nodays) = M_FLUXES(1:nodays,6)/M_POOLS(1:nodays,7)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    ...
    hak = 0 ; resid_root = 0d0
    resid_wood(1:nodays) = M_FLUXES(1:nodays,7)/M_POOLS(1:nodays,8)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
...
!                / sum(M_FLUXES(1:nodays,10))
    ...
    in_out_cwd  = sum(M_FLUXES(1:nodays,11)+disturbance_residue_to_cwd) / sum(M_FLUXES(1:nodays,19))
    in_out_som  = sum(M_FLUXES(1:nodays,15)+disturbance_residue_to_som) &
