...
module CARBON_MODEL_MOD
  ...
  public :: CARBON_MODEL            &
           ...
           ,wb_time                 &
		   ,rainfall_in_time        &
		   ,soil_waterfrac_2_time   &
		   ,soil_waterfrac_3_time   &
		   ,soil_waterfrac_1_m3m3_time &
		   ,soil_waterfrac_2_m3m3_time &
		   ,soil_waterfrac_3_m3m3_time &
		   ,water_grav_flow_2_time  &
		   ,water_grav_flow_3_time  &
		   ,infiltrated_L1_time     &
		   ,infiltrated_L2_time     &
		   ,infiltrated_L3_time     &
		   ,runoff_dew_time         &
		   ,Layer_thickness_time1   &
		   ,Layer_thickness_time2   &
  ...
  double precision, parameter :: pi = 3.1415927d0,  &
                ...
                sin_dayl_deg_to_rad = 0.3979486d0,  & ! sin( 23.45d0 * deg_to_rad )
                         ...
                         emissivity = 0.96d0,       & ! Surface emissivity (0-1)
                    ...
                    sw_par_fraction = 0.5d0,        & ! fraction of short-wave radiation which is PAR
                             freeze = 273.15d0,     & ! freezing point of water (K)
            ...
            partial_molar_vol_water = 18.05d-6,     & ! partial molar volume of water, m3 mol-1 at 20C
                   ...
                   mmol_to_kg_water = 1.8d-5,       & ! milli mole conversion to kg
                        ...
                        vonkarman_1 = 2.439024d0,   & ! 1 / von Karman's constant
                              ...
                              g_ms2 = 9.80665d0       ! Gravity in m/s2; used in soil water potential to convert from meters to KPa, and MPa.
  ...
  double precision, parameter :: &
                            ...
                            Vc_coef = 0.1408d0,     & ! Temperature above Vc_minT that 50% limitation of cold shutdown occurs
                                                      ! Assumption that photosynthesis will be limited by Jmax temperature response
                              ...
                              total_water_flux, & ! potential transpiration flux (kgH2O.m-2.day-1)
                                ...
                                previous_depth, & ! depth of bottom of soil profile
                                   canopy_wind, & ! wind speed (m.s-1) at canopy top
                                      ...
                                      ustar_Uh, & ! ratio of friction velocity to canopy top wind speed
                        ...
                        water_vapour_diffusion, & ! Water vapour diffusion coefficient in (m2/s)
                          ...
                          intercepted_rainfall, & ! intercepted rainfall rate equivalent (kgH2O.m-2.s-1)
                            soil_water_balance, & ! soil water balance
							    infiltrated_L1, & ! infiltration layer 1
								infiltrated_L2, & ! infiltration layer 2
...
metabolic_limited_photosynthesis, & ! temperature, leaf area and foliar N limiterd photosynthesis (gC/m2/day)
 ...
 leaf_canopy_light_scaling, & ! approximate scaling factor from leaf to canopy for gpp and gs
  ...
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,FLUXES,POOLS,DIAGS &
    ...
    integer :: harvest_management,n, iteration_count
	iteration_count = 0
  ...
  ! Printing the first value of each parameter for debugging
    ...
    if (.not.allocated(deltat_1)) then
        ...
        allocate(deltat_1(nodays),daylength_hours(nodays),daylength_seconds(nodays), &
				 ...
				 soil_waterfrac_2_time(nodays+1), soil_waterfrac_3_time(nodays+1), &
				 soil_waterfrac_1_m3m3_time(nodays+1), soil_waterfrac_2_m3m3_time(nodays+1), &
                 soil_waterfrac_3_m3m3_time(nodays+1), water_grav_flow_2_time(nodays), &
                 water_grav_flow_3_time(nodays), infiltrated_L1_time(nodays), &
				 infiltrated_L2_time(nodays), infiltrated_L3_time(nodays), &
        ...
        ! initialise some time invariant parameters
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
	...
	!print*, porosity 
	!print*, residual_waterfrac
	!print*, sat_conductivity
    !print*,	pore_size_dist
	!print*, air_entry
    ...
    soil_waterfrac_2_time(1) = 1d3 * soil_waterfrac(2) * layer_thickness(2)
	soil_waterfrac_3_time(1) = 1d3 * soil_waterfrac(3) * layer_thickness(3)
	soil_waterfrac_1_m3m3_time(1) = soil_waterfrac(1)
	soil_waterfrac_2_m3m3_time(1) = soil_waterfrac(2)
    soil_waterfrac_3_m3m3_time(1) = soil_waterfrac(3)
	
    do n = start, finish
	!print*,"swc begining loop",soil_waterfrac
  ...
  !         snow_melt = 0d0
   !    end if
    ...
    !end if
	   ...
	   soil_waterfrac_3_time(n+1) = 1d3 * soil_waterfrac(3) * layer_thickness(3)
	   soil_waterfrac_1_m3m3_time(n+1) = soil_waterfrac(1)
	   soil_waterfrac_2_m3m3_time(n+1) = soil_waterfrac(2)
       soil_waterfrac_3_m3m3_time(n+1) = soil_waterfrac(3)
	   water_grav_flow_2_time(n) = water_grav_flow(2)
	   water_grav_flow_3_time(n) = water_grav_flow(3)
	   infiltrated_L1_time(n) = infiltrated_L1
	   infiltrated_L2_time(n) = infiltrated_L2
	   infiltrated_L3_time(n) = infiltrated_L3 
	   runoff_dew_time(n) = runoff_dew
...
!    print*,"carbon_model: done"
  ...
  double precision function acm_gpp_stage_2(gs)
    ...
    ! Estimation of ci is based on the assumption that metabolic limited
    ...
    ! Daily canopy conductance determined through combination of aerodynamic and
    ...
    ! to make canopy resistance (s/m2/molCO2)
...
!    Sh_forced = 0.962d0*Pr_coef*(sqrt((leaf_width*canopy_wind)/kinematic_viscosity))
  ...
  subroutine calculate_field_capacity
    ...
    ! Subroutine to estimate field capacity (m3/m3)
    ...
    field_capacity = ((porosity - residual_waterfrac) * field_capacity) + residual_waterfrac
    
	!field_capacity(nos_soil_layers+1) = field_capacity(nos_soil_layers) 
	field_capacity(nos_soil_layers+1) = porosity(nos_soil_layers+1) * 0.99d0
...
!    endif
 ...
 !SOME THOUGHT NEEDED HERE ON HOW TO QUANTIFY THIS IMPACT ON GPLANT, AS STRESS VARIES DEPENDING ON THE ROOT ZONE LAYERS...
    ...
    do i = 1, nos_soil_layers
       ...
       call calculate_soil_conductivity(i,relative_water_frac(i),soil_conductivity(i))
    end do ! soil layers
...
!                      due to the non-linear effects on soil surface evaporation as a result of drythick development
    ...
    water_grav_flow = water_grav_flow * days_per_step_1
    infiltrated = infiltrated * days_per_step_1
	...
	infiltrated_L1 = infiltrated_L1 * days_per_step_1
	infiltrated_L2 = infiltrated_L2 * days_per_step_1
    infiltrated_L3 = infiltrated_L3 * days_per_step_1
    runoff_dew = runoff_dew * days_per_step_1
	
    ! Based on the soil mass balance corrected_ET, make assumptions to correct Eleaf and Esnow
    soil_water_balance = (corrected_ET-Eleaf) / Esoil
    if (soil_water_balance < 1d0 .and. soil_water_balance > 0d0) Esoil = Esoil * soil_water_balance
    ...
    if (root_reach > depth_change ) then
       ...
       depth_change_Layer = depth_change
    ...
    water_change_Layer = water_change * 1d3
    ...
    soil_water_balance = soil_water_balance &
                       - (sum(soil_waterfrac(1:nos_soil_layers) * layer_thickness(1:nos_soil_layers) * 1d3) - initial_soilwater)
	

  if (abs(soil_water_balance) > 1d-6 .or. soil_waterfrac(1) < -1d-6) then
       
	  print*,"Water Balance (mm)",soil_water_balance
...
!        print*,"Soil Water Fraction = ",soil_waterfrac
  ...
  subroutine infiltrate(rainfall_in)
	...
	!print*, "rainfall_in 1", rainfall_in
  ...
  !  do i = 1, nos_soil_layers
	...
	!infiltrated = infiltrated + (waterchange(1) * 1d3)
    infiltrated = infiltrated + (sum(waterchange) * 1d3)
	infiltrated_L1 = infiltrated_L1 + (waterchange(1) * 1d3)
	infiltrated_L2 = infiltrated_L2 + (waterchange(2) * 1d3)
	infiltrated_L3 = infiltrated_L3 + (waterchange(3) * 1d3 )
  ...
  subroutine gravitational_drainage(time_period_days)
    ...
    double precision, dimension(nos_soil_layers) :: dx, & ! range between the start and end points of the integration
                                         ...
                                         avail_to_flow, & ! liquid content above field capacity (m3/m3)
                                          ...
                                          pot_drainage    ! estimates of time step potential drainage rate (m/s)
...
!	print*,"start of grav drainage "
 ...
 !   print*,"avail_to_flow",avail_to_flow
...
!       
  ...
  !-----------------------------------------------------------------
  !
  subroutine calculate_relative_water_frac(s, waterfrac, relative_water)
  ...
  subroutine initialise_soils
    ...
    ! Units: (-)
	m_pore_size_dist = 1.0d0 - (1.0d0 / pore_size_dist)
  ...
  !-----------------------------------------------------------------
  !
  subroutine calculate_soil_conductivity(s,relative_water_frac_local,conductivity)
	...
	conductivity = sat_conductivity(s) * (relative_water_frac_local**0.5d0) * &
                 (1.0d0 - (1.0d0 - relative_water_frac_local**(1.0d0/m_pore_size_dist(s)))**m_pore_size_dist(s))**2d0
...
!    end if
  ...
  double precision function water_retention_eqns( xin )
   ...
   soil_wp = g_ms2 * (-1.0d0 / air_entry(water_retention_pass)) * &
	           (xin**(-1.d0/m_pore_size_dist(water_retention_pass)) - 1.d0)**(1.d0/pore_size_dist(water_retention_pass))
