...
!    end if ! abs(balance) > 1d-10
  ...
  subroutine calculate_soil_conductivity(soil_layer,waterfrac,conductivity)
	...
	conductivity = sat_conductivity(soil_layer) * (relative_water_frac(soil_layer)**0.5d0) *&
					(1.0d0 - (1.0d0 - relative_water_frac(soil_layer)**(1.0d0/m_pore_size_dist(soil_layer)))**&
					m_pore_size_dist(soil_layer))**2d0
  ...
  subroutine soil_water_potential
    ...
    SWP(1:nos_soil_layers) = (g_ms2*1.0E-3) * (-1.0d0 / air_entry(1:nos_soil_layers)) *&
							 (relative_water_frac(1:nos_soil_layers)**(-1.0d0 / m_pore_size_dist(1:nos_soil_layers)) - 1.0d0)&
  ...
  double precision function water_retention_eqns( xin )
   ...
   ! Subroutine to calculate soil water potential (SWP) using the Van-Genuchten Mualem (VGM) Model
	...
	soil_wp = g_ms2 * (-1.0d0 / air_entry(water_retention_pass)) * &
			(relative_water_frac_local**(-1.d0/m_pore_size_dist(water_retention_pass)) - 1.d0)**&
			(1.d0/pore_size_dist(water_retention_pass))
