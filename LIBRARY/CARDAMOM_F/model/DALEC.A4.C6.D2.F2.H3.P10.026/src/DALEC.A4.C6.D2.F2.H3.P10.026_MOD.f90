...
! This file contains the parameters and memory of DALEC.A4.C6.D2.F2.H3.P10.026
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  double precision, parameter :: pi = 3.1415927d0,  &
                ...
                sin_dayl_deg_to_rad = 0.3979486d0,  & ! sin( 23.45d0 * deg_to_rad )
                    ...
                    sw_par_fraction = 0.5d0,        & ! fraction of short-wave radiation which is PAR
                             freeze = 273.15d0,     & ! freeze / melt temperature of water (K)
...
!                         iWUE = 4.6875d-4,     & ! Intrinsic water use efficiency (umolC/mmolH2O-1/m2leaf/s-1)
  ...
  integer, parameter :: max_leaf_cohorts = 96

  ! Derived type for a single leaf cohort, array version of this type will be created to represent the whole structure.
  ! After its initialisation, the exact position in the array will no longer infer the actual age. This saves on computational
  ! effort of maintaining the structure.
  type :: cohort_leaf_t
      integer :: age_days   = 0, &   ! days elapsed since birth
                 age_months = 0      ! ineger months elapsed (age_days/30)
      double precision :: Cf             = 0d0, & ! leaf carbon (gC m-2)
                          Cc             = 0d0, & ! construction cost at birth (gC m-2)
                          cum_profit     = 0d0, & ! cumulative NCCE since birth (gC m-2)
                          NUE_rel        = 1d0, & ! relative NUE [0-1], declines with age
                          ncce_threshold = 0d0, & ! economic shedding threshold (gC gC-1 m-2 d-1)
                          profit_target  = 0d0, & ! cumulative profit target (gC gC-1 m-2 LL)
                          ncce_average   = 0d0    ! mean annual ncce (gC/m2/day)
      logical          :: is_alive = .false.
  end type cohort_leaf_t


  ! Minimum interval between consecutive cohort creation events (days).
  double precision, parameter :: cohort_birth_period = 30d0
  ...
  type(cohort_leaf_t), dimension(max_leaf_cohorts) :: leaf_cohorts
  integer             :: n_live_cohorts = 0, & ! Number of living cohorts
                   n_profitable_cohorts = 0    ! Number of cohorts which are in the economic black
  ...
  double precision    :: days_since_last_cohort = 0d0
...
metabolic_limited_photosynthesis, & ! temperature, leaf area and foliar N limiterd photosynthesis (umolC/m2/s)
    light_limited_photosynthesis, & ! light limited photosynthesis (umolC/m2/s)
                dark_respiration, & ! leaf dark respiration (umolC/m2/s)
                   cold_shutdown, & ! Cold response shutdown of leaf metabolitc activity (0-1)
 ...
 leaf_canopy_light_scaling, & ! approximate scaling factor from leaf to canopy for gpp and gs
  ...
  double precision, dimension(:), allocatable :: deltat_1, & ! inverse of decimal days
                                  airt_zero_fraction_time, & ! fraction of temperature above zero for each time step
                                            rainfall_time, & ! Realism checked rainfall
                                          daylength_hours, & ! Store of day lengths in hours
                                        daylength_seconds, & ! Store of day lengths in seconds
                                      daylength_seconds_1    ! Inverse of store of day lengths in seconds
  end type
