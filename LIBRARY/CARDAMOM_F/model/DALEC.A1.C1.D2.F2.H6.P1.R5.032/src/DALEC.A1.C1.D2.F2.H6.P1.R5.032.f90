...
! This file contains the source code of DALEC.A1.C1.D2.F2.H6.P1.R5.032
!
...
module CARBON_MODEL_MOD
  ...
  integer, parameter :: nos_root_layers = 1, nos_soil_layers = nos_root_layers + 1
...
!print*, "begining test"
    ...
    do n = start, finish
	   ...
	   porosity_time_L1(n) = porosity(1)
       porosity_time_L2(n) = porosity(2)
	   field_capacity_time_L2 = field_capacity(2)
	  ...
	  iteration_count = iteration_count + 1
       ...
       FLUXES(n,50) = infiltrated(2)  ! middle soil surface infiltration by rain (kgH2O/m2/day)
       FLUXES(n,51) = infiltrated(2)  ! bottom soil surface infiltration by rain (kgH2O/m2/day)       
...
!print*, "end of test"
 ...
 ! Printing or passing to output variable?
	   ...
	   soil_waterfrac_total_time(n+1) = sum(soil_waterfrac(1:nos_soil_layers) * layer_thickness(1:nos_soil_layers) * 1d3)
...
!    Sh_forced = 0.962d0*Pr_coef*(sqrt((leaf_width*canopy_wind)/kinematic_viscosity))
  ...
  subroutine calculate_field_capacity
	...
	field_capacity(nos_soil_layers+1) = field_capacity(2)
...
!
  !  if (abs(soil_water_balance) > 1d-6 .or. soil_waterfrac(1) < -1d-6) then
   !     print*,"Soil water miss-balance (mm)",soil_water_balance
...
!        print*,"Soil Water Fraction = ",soil_waterfrac
  ...
  subroutine infiltrate(rainfall_in)
	...
	!print*, "add 2 ", add
	!print*, "pot-r 1", pot_runoff
	!print*, "runoff 1", runoff
	...
	!print*, "add 3", add
	!print*, "pot-r 2", pot_runoff
	!print*, "runoff 2", runoff
	
    runoff = runoff + (add * 1d3)
	!print*, "runoff 3", runoff
...
!  end subroutine gravitational_drainage
  ...
  subroutine gravitational_drainage(time_period_days)
    ...
    ! Reset
    liquid_fraction = 1d0 ; waterchange = 0d0 ; d = 1

    ! calculate soil ice proportion;
    ! except the surface layer in the mean daily temperature is < 0oC
    if (meant < 1d0) liquid_fraction(1) = 0d0
    ...
    ! Estimate liquid content and how much is available to flow / drain
    avail_to_flow = (soil_waterfrac(1:nos_soil_layers) * liquid_fraction(1:nos_soil_layers) ) &
                  - field_capacity(1:nos_soil_layers)
    ...
    do while (d < 4 .and. maxval(avail_to_flow) > vsmall) 
        ...
        do s = 1, nos_soil_layers
           ...
           if (avail_to_flow(s) > 0d0 .and. soil_waterfrac(s+1) < porosity(s+1)) then
               ...
               soil_waterfrac_local = soil_waterfrac
               ...
               call calculate_relative_water_frac(s,liquid,tmp4)
               call calculate_soil_conductivity(s,tmp4,pot_drainage_k1)  
               ...
               call gravitational_drainage_local_update(s,soil_waterfrac_local,pot_drainage_k0,liquid_fraction(s), &
                                                        layer_thickness,field_capacity(s),porosity)
               ! Estimate the soil water conductance at this new state
               liquid = (soil_waterfrac_local(s) * liquid_fraction(s) )      
               call calculate_relative_water_frac(s,liquid,tmp4)
               call calculate_soil_conductivity(s,tmp4,pot_drainage_k2)  
               ...
               liquid = (soil_waterfrac_local(s) * liquid_fraction(s) )         
               call calculate_relative_water_frac(s,liquid,tmp4)
               call calculate_soil_conductivity(s,tmp4,pot_drainage_k3)  
               ...
               liquid = (soil_waterfrac_local(s) * liquid_fraction(s) )         
               call calculate_relative_water_frac(s,liquid,tmp4)
               call calculate_soil_conductivity(s,tmp4,pot_drainage_k4)  
               ...
               ! Unsaturated volume of layer below (m3 m-2)
               unsat = ( porosity(s+1) - soil_waterfrac(s+1) ) * layer_thickness(s+1) / layer_thickness(s)
               ! Restrict potential rate calculate above for the available water
               ! and available space in the layer below.
               ! NOTE: * layer_thickness(s) converts units from m3/m2 -> (m3)
               change = min(unsat,min(pot_drainage_k0,avail_to_flow(s))) * layer_thickness(s)
               ! update soil layer below with drained liquid
               waterchange( s + 1 ) = waterchange( s + 1 ) + change
               waterchange( s     ) = waterchange( s     ) - change
               ! Also track only the positive flows from one layer to another (MgH2O/m2/day)
               water_grav_flow(s) = water_grav_flow(s) + change
               ...
               soil_waterfrac(s:(s+1)) = soil_waterfrac(s:(s+1)) + (waterchange(s:(s+1))/layer_thickness(s:(s+1)))
  ...
  subroutine initialise_soils(input_porosity, input_residual_waterfrac, input_saturated_conductivity, &
                              input_pore_size_dist, input_air_entry, input_soilwater_frac)
    ...
    porosity(1) = input_porosity 
    residual_waterfrac(1) = input_residual_waterfrac
    saturated_conductivity(1) = input_saturated_conductivity
    pore_size_dist(1) = input_pore_size_dist
    air_entry(1) = input_air_entry
	
	
	! Assign VGM parameters for Catotelm layer
	porosity(2)=  0.8d0
	residual_waterfrac(2) = 0.15d0
	saturated_conductivity(2) = 5.0d-8
	pore_size_dist(2) = 1.19d0
	air_entry(2) = 1.6d0
    ...
    ! Units: (-)
	m_pore_size_dist = 1d0 - (1d0 / pore_size_dist)
  ...
  subroutine calculate_soil_conductivity(s,relative_water_frac_local,conductivity)
	...
	conductivity = saturated_conductivity(s) * (relative_water_frac_local**pore_connectivity) * &
                 (1d0 - (1d0 - relative_water_frac_local**(1d0/m_pore_size_dist(s)))**m_pore_size_dist(s))**2d0
...
!    end if
  ...
  double precision function water_retention_eqns( xin )
   ...
   soil_wp = 1d3 * head * (-1d0 / air_entry(water_retention_pass)) * &
	           (xin**(-1d0/m_pore_size_dist(water_retention_pass)) - 1d0)**(1d0/pore_size_dist(water_retention_pass))
