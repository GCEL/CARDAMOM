...
!                   pn_opt_temp = 3.155960d+01,  & ! Minimum daily max temperature for photosynthesis (oC)
...
! Assumption that photosythesis will be limited by Jmax temperature response
                   pn_max_temp = 57.05d0,		   &		!57.05d0,      & ! Maximum daily max temperature for photosynthesis (oC) | TG previous V4/V5 45
                   pn_min_temp = -7.41d0,      & !def = -1d6! Minimum daily max temperature for photosynthesis (oC) | TG previous V4/V5/99 -7.41d0
                   pn_opt_temp = 30d0,		   &		!30d0,         & ! Optimum daily max temperature for photosynthesis (oC) | TG previous V4/V5 22
                   pn_kurtosis = 0.1d0,	   &	!def = 0.172d0,      & ! Kurtosis of Jmax temperature response | TG previous V4/V5 0.13 99 = 0.1
...
!    print*,"carbon_model: "
    ...
    ! the assumption that the fine and coarse roots are mechanically removed.
    ! 1 = all removed, 0 = all remains.
    roots_frac_removal(2)  = 0.95d0 ! assume clear cut with 5% retention = 0.95d0; 0% retention = 1.0d0
    rootcr_frac_removal(2) = 0.95d0  ! 5% retention = 0.95d0; 0% retention = 1.0d0
    ! harvest residue (fraction); 1 = all remains, 0 = all removed
    foliage_frac_res(2) = 1d0 ! 1d0 = all foliage removed by harvest is left on site
    roots_frac_res(2)   = 1d0 !1d0 = stumps and roots (fine and coarse) assumed to stay on site 
    rootcr_frac_res(2) = 1d0 ! assume stumps stay in ground
    stem_frac_res(2)   = 0.175d0 ! 0.175d0 from Finland MS-NFI, stem_frac_res = Cbranch / (Cbranch+Cstem) ---> branches often left at site following clear cut
    ! wood partitioning (fraction)
    Crootcr_part(2) = 0.246d0 ! Coarse roots From Finland MS-NFI. Here, coarse root includes stump;
    ! Csom loss due to phyical removal with roots
    ! Morison et al (2012) Forestry Commission Research Note
    soil_loss_frac(2) = 0d0! DEF = 0.02d0 ! actually between 1-3 %, but here set to 0 bc we assume no stump extraction and harvest in winter -> frozen soils -> min impact on soil
    ! was the forest burned after deforestation (0-1)
    ! NOTE: that we refer here to the fraction of the cleared land to be burned
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       !!!!!!!!!!
      ! print*,"airt_zero_fraction = ", airt_zero_fraction, "at timestep: ",n
	  ! print*,"precip = ", rainfall*2629800d0, "at timestep: ",n
       ! snowing or not...?
	...
	! print*,"snowfall = ", snowfall*2629800d0, "at timestep: ",n
	! print*,"rainfall = ", rainfall*2629800d0, "at timestep: ",n
	...
	!print*,"snow storage = ", snow_storage, "at timestep: ",n
       !!!!!!!!!!
       ...
       if (met(8,n) > 0d0) then
           ...
           if (C_total > 0d0) then
               ...
               labile_frac_removal = ((POOLS(n+1,3)/C_total) * roots_frac_removal(harvest_management)  ) &
                                     + ((Cstem/C_total)        * 1.0d0   ) & ! 5% retention TG = 0.95d0, 1d0 = 0% retention
               ...
               foliar_loss = POOLS(n+1,2) * met(8,n) * 0.95d0 ! 5% retention clear cut TG https://www.sciencedirect.com/science/article/pii/S037811272300378X
               roots_loss  = POOLS(n+1,3) * roots_frac_removal(harvest_management) * met(8,n)
               stem_loss   = (Cstem * 0.95d0 * met(8,n)) ! 5% retention = 0.95d0; 0% retention = 1.0d0 TG
...
!    print*,"carbon_model: done"
  ...
  subroutine calculate_stomatal_conductance
    ...
    if (aerodynamic_conductance > vsmall .and. total_water_flux > vsmall ) then !.and. leafT > Vc_minT
        ...
        !minimum_conductance = min_gs * leaf_canopy_light_scaling * dayl_seconds
