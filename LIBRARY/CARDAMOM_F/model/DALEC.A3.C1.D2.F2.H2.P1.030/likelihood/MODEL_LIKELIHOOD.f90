...
! of DALEC.A3.C1.D2.F2.H2.P1 as a function of observations and ecological dynamical constraints.
!
...
!    end do
  ...
  subroutine assess_EDC1(PARS, npars, meantemp, meanrad, EDC1, EDCD)
    ...
    ! Canopy / soil optical properties

    ! Sum of NIR reflectance and transmittance must be less than 1
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(33) + pars(35) > 1d0) then
        EDC1 = 0d0 ; EDCD%PASSFAIL(9) = 0
    endif
    ! Sum of PAR reflectance and transmittance must be less than 1
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(34) + pars(36) > 1d0) then
        EDC1 = 0d0 ; EDCD%PASSFAIL(10) = 0
    endif
    ! Canopy reflectance must for NIR should be greater than PAR
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(33) < pars(34)) then
        EDC1 = 0d0 ; EDCD%PASSFAIL(11) = 0
    endif
    ! Canopy transmittance must for NIR should be greater than PAR
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(35) < pars(36)) then
        EDC1 = 0d0 ; EDCD%PASSFAIL(12) = 0
    endif
    ! Soil reflectance must for NIR should be greater than PAR
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(37) < pars(38)) then
  ...
  subroutine calc_other_likelihoods(ML_obs_out, M_POOLS, M_FLUXES, M_DIAGS)
    ...
    if (DATAin%otherpriors(5) > -9998) then
        ...
        mod = (sum(M_FLUXES(1:DATAin%nodays,7))/dble(DATAin%nodays)) & 
            * ((sum(M_POOLS(1:DATAin%nodays,4)/(M_FLUXES(1:DATAin%nodays,11)+M_FLUXES(1:DATAin%nodays,25)))) &
              / dble(DATAin%nodays))
