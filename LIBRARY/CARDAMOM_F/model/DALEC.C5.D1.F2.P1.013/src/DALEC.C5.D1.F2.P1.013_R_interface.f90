...
! Subroutine to allow direct interface between DALEC.C5.D1.F2.P1.013 and the R code
!
...
subroutine rdalec13(output_dim,MTT_dim,SS_dim &
  ...
  call initialize_mv(mV, nodays, nomet, nopars, deltat, (/60d0, 60d0/), (/30d0, 30d0/), met, lat)
...
!endif
     ...
     out_var1(i,1:nodays,5)  = FLUXES(1:nodays,4)       ! allocation to foliage (gC/m2/day)
     out_var1(i,1:nodays,6)  = FLUXES(1:nodays,5)       ! allocation to labile (gC/m2/day)
     out_var1(i,1:nodays,7)  = FLUXES(1:nodays,6)       ! allocation to wood + fine roots (gC/m2/day)
     out_var1(i,1:nodays,8)  = FLUXES(1:nodays,8)       ! labile to foliage (gC/m2/day)
     out_var1(i,1:nodays,9)  = FLUXES(1:nodays,10)      ! foliage to litter (gC/m2/day)
     out_var1(i,1:nodays,10) = FLUXES(1:nodays,11)      ! wood + roots to som (gC /m2/day)
     ! C disturbance fluxes (gC/m2/day)
     out_var1(i,1:nodays,11) = FLUXES(1:nodays,18)      ! fire emission from labile (gC/m2/day)
     out_var1(i,1:nodays,12) = FLUXES(1:nodays,22)      ! fire induced litter from labile (gC/m2/day)
     out_var1(i,1:nodays,13) = FLUXES(1:nodays,19)      ! fire emission from foliage (gC/m2/day)
     out_var1(i,1:nodays,14) = FLUXES(1:nodays,23)      ! fire induced litter from foliage (gC/m2/day)
     out_var1(i,1:nodays,15) = FLUXES(1:nodays,20)      ! fire emission from fine roots and wood (gC/m2/day)
     out_var1(i,1:nodays,16) = FLUXES(1:nodays,24)      ! fire induced litter from fine roots + wood(gC/m2/day)
     out_var1(i,1:nodays,17) = FLUXES(1:nodays,21)      ! fire emission from litter + som (gC/m2/day)
     ! C pools (gC/m2)
     out_var1(i,1:nodays,18) = POOLS(1:nodays,1)        ! labile (gC/m2)
     out_var1(i,1:nodays,19) = POOLS(1:nodays,2)        ! foliage (gC/m2)
     out_var1(i,1:nodays,20) = POOLS(1:nodays,3)        ! fine root + wood (gC/m2)
     out_var1(i,1:nodays,21) = POOLS(1:nodays,4)        ! litter + som (gC/m2)
     ! Canopy (phenology) properties
     out_var1(i,1:nodays,22) = DIAGS(1:nodays,1)        ! LAI (m2/m2)
     out_var1(i,1:nodays,23) = DIAGS(1:nodays,2)        ! ratio of leaf internal to external CO2
     ...
     where (POOLS(1:nodays,1) > 0d0) ! protection against NaN from division by zero
            ...
            tmp = ((FLUXES(1:nodays,8) + &
                    ...
                    FLUXES(1:nodays,26) + FLUXES(1:nodays,30)) / POOLS(1:nodays,1))
     end where
     ...
     where (POOLS(1:nodays,2) > 0d0) ! protection against NaN from division by zero
            ...
            tmp = ((FLUXES(1:nodays,10) + &
                    ...
                    FLUXES(1:nodays,27) + FLUXES(1:nodays,31)) / POOLS(1:nodays,2))
     end where
     ...
     where (POOLS(1:nodays,3) > 0d0) ! protection against NaN from division by zero
            ...
            tmp = ((FLUXES(1:nodays,11) + &
                    FLUXES(1:nodays,20) + FLUXES(1:nodays,24) + &
                    FLUXES(1:nodays,28) + FLUXES(1:nodays,32)) / POOLS(1:nodays,3))
     end where
     ...
     where (POOLS(1:nodays,4) > 0d0) ! protection against NaN from division by zero
            pool_hak = 0 
            tmp = ((FLUXES(1:nodays,13) + FLUXES(1:nodays,21) + &
                    FLUXES(1:nodays,29)) / POOLS(1:nodays,4))
     end where
  ...
  out_var3(:,1:3) = out_var3(:,1:3) * out_var2(:,1:3) ! multiply by residence time in years
  ! Using the wood SS estimate (gC/m2) the steady state input to the som litter pool...
  out_var3(:,4) = (out_var3(:,4) + (out_var3(:,3) / out_var2(:,3))) * out_var2(:,4)

  ! return back to the subroutine then
  return

end subroutine rdalec13
