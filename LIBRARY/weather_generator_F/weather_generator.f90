module weather_generator
  ...
  subroutine calc_sw( day_number, days_in_year , latitude , sw_day , declination , sunrise , daylength , sw_hr )
    ...
    maxhr = min(nint(hours_per_day),floor(sunrise+daylength))
  ...
  subroutine calc_temp( tmax_day , tmin_day , tavg_day , sunrise , t_hr )
    ...
    do hr = 1 , int(sunrise)
       ...
       t_hr( hr ) =  ( 1.0 / ( sunrise - 0.0 ) ) * &
                ( tmin_day * ( real(hr) - 0.0 ) + sat_at_midnight * ( sunrise - real(hr) ) )
    enddo
    ...
    do hr = int(sunrise)+1 , index_hot_time
       ...
       t_hr( hr ) = ( 1.0 / ( hottest_time - sunrise ) ) * &
                ( tmax_day * ( real(hr) - sunrise ) + tmin_day * ( hottest_time - real(hr) ) )
    enddo
    ...
    do hr = int(hottest_time)+1, nint(hours_per_day)
       ...
       t_hr(hr) = ( 1.0 / ( hours_per_day - hottest_time ) ) * &
                  (sat_at_midnight * ( real(hr) - hottest_time ) + tmax_day * ( hours_per_day - real(hr) ))
    enddo
  ...
  subroutine calc_rh( rh_max , rh_min , rh_avg , sunrise , rh_hr )
    ...
    do hr = 1 , int(sunrise)
       ...
       rh_hr(hr) =  ( 1.0 / ( sunrise - 0.0 ) ) * &
                    ( rh_max * ( real(hr) - 0.0 ) + rh_at_midnight * ( sunrise - real(hr) ) )
    ...
    do hr = int(sunrise)+1 , index_hot_time
       ! straight line from Tmin to Tmax
       rh_hr( hr ) = ( 1.0 / ( hottest_time - sunrise ) ) * &
                     ( rh_min * ( real(hr) - sunrise ) + rh_max * ( hottest_time - real(hr) ) )
       ! sanity check however
       rh_hr( hr ) = min(1.0,max(0.0, rh_hr(hr)))
    enddo
    ...
    sum_one = 0.0 ; sum_two = 0.0
    do hr = index_hot_time+1 , nint(hours_per_day)
       sum_one = sum_one + ( real(hr) - hottest_time )
       sum_two = sum_two + ( hours_per_day - real(hr) )
    enddo
    rh_at_midnight = ( 1.0 / sum_one) * ( &
    ...
    do hr = int(hottest_time)+1, nint(hours_per_day)
       ...
       rh_hr( hr ) = ( 1.0 / ( hours_per_day - hottest_time ) ) * &
                     ( rh_at_midnight * ( real(hr) - hottest_time ) + rh_min * ( hours_per_day - real(hr) ) )
       ! sanity check however
       rh_hr( hr ) = min(1.0, max(0.0, rh_hr(hr)))
    enddo
  ...
  function calc_ppt( ppt_day, sunrise )
    ...
    ! Reset the precipitation counter
    calc_ppt = 0.0
