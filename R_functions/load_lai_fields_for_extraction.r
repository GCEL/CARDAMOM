...
#WHERE TO DETERMINE THE DOMAIN WE WANT TO READ IN SO REDUCE THE AMOUNT OF TIME SPEND LOADING?
           ...
           if (length(this_year) > 0) {
               ...
               for (t in seq(1, length(this_year))) {
                    ...
                    if (length(which(grepl("latitude",names(data1$dim)) == TRUE)) > 0) {
                        lat_key = "latitude"
                    } else if (length(which(grepl("lat",names(data1$dim)) == TRUE)) > 0)  {
                        lat_key = "lat"
                    } else if (length(which(grepl("Latitude",names(data1$dim)) == TRUE)) > 0)  {
                        lat_key = "Latitude"
                    } else if (length(which(grepl("Lat",names(data1$dim)) == TRUE)) > 0)  {
                        lat_key = "Lat"
                    } else {
                    ...
                    if (length(which(grepl("longitude",names(data1$dim)) == TRUE)) > 0) {
                        lon_key = "longitude"
                    } else if (length(which(grepl("long",names(data1$dim)) == TRUE)) > 0)  {
                        lon_key = "long"
                    } else if (length(which(grepl("Longitude",names(data1$dim)) == TRUE)) > 0)  {
                        lon_key = "Longitude"
                    } else if (length(which(grepl("Long",names(data1$dim)) == TRUE)) > 0)  {
                        lon_key = "Long"
                    } else if (length(which(grepl("Lon",names(data1$dim)) == TRUE)) > 0)  {
                        lon_key = "Lon"
                    } else if (length(which(grepl("lon",names(data1$dim)) == TRUE)) > 0)  {
                        lon_key = "lon"
                    } else {
