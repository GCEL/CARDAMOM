...
load_biomass_stocks_maps_for_extraction<-function(latlon_in,Cwood_stock_source,start,finish,timestep_days,cardamom_ext,spatial_type) {
    ...
    if (Cwood_stock_source == "mpi_biomass") {
    ...
    } else if (Cwood_stock_source == "ESA_CCI_Biomass_stratified") {

       # let the user know this might take some time
       print("Loading processed ESA-CCI biomass layers for subsequent sub-setting ...")

       # Create the full file paths to both 2010 and 2017 AGB estimates
       input_file = Sys.glob(paste(path_to_Cwood,"AGBiomass_stocks_2010_with_lat_lon*.nc",sep=""))
       input_file = append(input_file,Sys.glob(paste(path_to_Cwood,"AGBiomass_stocks_2017_with_lat_lon*.nc",sep="")))
       input_file = append(input_file,Sys.glob(paste(path_to_Cwood,"AGBiomass_stocks_2018_with_lat_lon*.nc",sep="")))
       input_file = append(input_file,Sys.glob(paste(path_to_Cwood,"AGBiomass_stocks_2019_with_lat_lon*.nc",sep="")))
       input_file = append(input_file,Sys.glob(paste(path_to_Cwood,"AGBiomass_stocks_2020_with_lat_lon*.nc",sep="")))
       years_with_obs = c(2010,2017,2018,2019,2020)
       ...
       for (t in seq(1, length(years_with_obs))) {
            ...
            if ((years_with_obs[t] >= as.numeric(start)) & (years_with_obs[t] <= as.numeric(finish))) {
                ...
                biomass = biomass[keep_long,keep_lat]
                biomass_uncertainty = biomass_uncertainty[keep_long,keep_lat]
